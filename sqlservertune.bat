...
set title=SqlServerTune-console
title %title%

rem ------------------------------------------------------------------------
rem --- set SQLSERVERTUNE_HOME to current directory if NOT already set
rem ------------------------------------------------------------------------
IF "%SQLSERVERTUNE_HOME%"=="" set SQLSERVERTUNE_HOME=%~dp0
...
rem ------------------------------------------------------------------------
rem --- remove last '\' char from: SQLSERVERTUNE_HOME and SYBASE
rem --- SQLSERVERTUNE_HOME exists every time, while SYBASE is NOT mandatory
rem --- Also for SYBASE
...
rem ------------------------------------------------------------------------
IF %SQLSERVERTUNE_HOME:~-1%==\ SET SQLSERVERTUNE_HOME=%SQLSERVERTUNE_HOME:~0,-1%
...
rem ------------------------------------------------------------------------
set APPL_HOME=%SQLSERVERTUNE_HOME%
set SQLSERVERTUNE_SAVE_DIR=%SQLSERVERTUNE_HOME%\data
...
rem ------------------------------------------------------------------------
echo NOTE: Set/Change JVM Memory parameters by setting Environment variable: SQLSERVERTUNE_JVM_MEMORY_PARAMS 

set JVM_MEMORY_PARAMS_32=-Xmx1024m -Xms64m
set JVM_MEMORY_PARAMS_64=-Xmx2048m -Xms64m

IF DEFINED SQLSERVERTUNE_JVM_MEMORY_PARAMS set JVM_MEMORY_PARAMS_32=%SQLSERVERTUNE_JVM_MEMORY_PARAMS%
IF DEFINED SQLSERVERTUNE_JVM_MEMORY_PARAMS set JVM_MEMORY_PARAMS_64=%SQLSERVERTUNE_JVM_MEMORY_PARAMS%
...
IF DEFINED SQLSERVERTUNE_JVM_GC_PARAMS set JVM_GC_PARAMS_32=%SQLSERVERTUNE_JVM_GC_PARAMS%
IF DEFINED SQLSERVERTUNE_JVM_GC_PARAMS set JVM_GC_PARAMS_64=%SQLSERVERTUNE_JVM_GC_PARAMS%
...
IF NOT DEFINED SQLSERVERTUNE_HOME GOTO no_sqlservertunehome
...
rem ------------------------------------------------------------------------
set classpath=%SQLSERVERTUNE_HOME%\classes
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\asetune.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\jconn4.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\jconn3.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\jtds-1.2.7.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\ngdbc.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\dsparser.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\log4j-1.2.17.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\h2-1.3.176.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\wizard.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\miglayout-swing-4.2.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\miglayout-core-4.2.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\swingx-all-1.6.5-1.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\jchart2d-3.2.2.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\planviewer.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\commons-cli-1.2.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\proxy-vole_20131209.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\ganymed-ssh2-build251beta1.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\rsyntaxtextarea.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\autocomplete.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\rstaui.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\jcommon-1.0.21.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\jfreechart-1.0.17.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\antlr-4.0-complete.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\juniversalchardet-1.0.3.jar
set classpath=%classpath%;%SQLSERVERTUNE_HOME%\lib\DDLGen.jar
...
rem ------------------------------------------------------------------------
set PATH=%SQLSERVERTUNE_JAVA_HOME%\bin;%JAVA_HOME%\bin;%PATH%
...
if %JavaBitness% == 64 (
	...
	echo NOTE: Java is a 64 bit, SqlServerTune will be allowed to use more memory
)
...
rem ------------------------------------------------------------------------
pushd %SQLSERVERTUNE_HOME%
REM echo %CLASSPATH%

java %JVM_MEMORY_PARAMS% %JVM_GC_PARAMS% %JVM_PARAMS% -Dsybase.home="%SYBASE%" -DSYBASE="%SYBASE%" -DAPPL_HOME="%SQLW_HOME%" -DSQLSERVERTUNE_HOME="%SQLSERVERTUNE_HOME%" -DSQLSERVERTUNE_SAVE_DIR="%SQLSERVERTUNE_SAVE_DIR%" %EXTRA% %DEBUG_OPTIONS% %SPLASH% com.asetune.SqlServerTune %*

IF %ERRORLEVEL% NEQ 0 GOTO unexpected_error
goto exit_sqlservertune
...
:no_sqlservertunehome
echo -----------------------------------------------------------------------
echo Error: no SQLSERVERTUNE_HOME environment variable.
echo -----------------------------------------------------------------------
echo Must set the SQLSERVERTUNE_HOME variable to the place where you installed SqlServerTune.
echo -----------------------------------------------------------------------
goto exit_sqlservertune
...
echo Current SQLSERVERTUNE_JAVA_HOME variable is set to %SQLSERVERTUNE_JAVA_HOME%
...
echo Subject: SqlServerTune starting problem
echo -----------------------------------------------------------------------
echo .
goto exit_sqlservertune
...
echo .
echo If you have a sql.ini file somewhere, SqlServerTune looks for it under %%SYBASE%%\ini\sql.ini.
