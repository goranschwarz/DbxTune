...
IF NOT DEFINED DBXTUNE_HOME GOTO no_dbxtunetunehome
...
rem ----set classpath=%classpath%;%DBXTUNE_HOME%\lib\sqljdbc4.jar
...
set classpath=%classpath%;%DBXTUNE_HOME%\lib\h2-1.4.193.jar
...
set classpath=%classpath%;%DBXTUNE_HOME%\lib\jsqlparser-0.9.6.jar
...
set MSSQL_AUTH_PATH=
if %JavaBitness% == 64 (
	set MSSQL_AUTH_PATH=%DBXTUNE_HOME%\lib\jdbc_drivers\sqlserver_auth\x64
) else (
	set MSSQL_AUTH_PATH=%DBXTUNE_HOME%\lib\jdbc_drivers\sqlserver_auth\x86
)
...
java %JVM_MEMORY_PARAMS% %JVM_GC_PARAMS% %JVM_PARAMS% -Duser.language=en -Dsybase.home="%SYBASE%" -DSYBASE="%SYBASE%" -DAPPL_HOME="%SQLW_HOME%" -DDBXTUNE_HOME="%DBXTUNE_HOME%" -DDBXTUNE_SAVE_DIR="%DBXTUNE_SAVE_DIR%" %EXTRA% %DEBUG_OPTIONS% %SPLASH% com.asetune.SqlServerTune %*

IF %ERRORLEVEL% NEQ 0 GOTO unexpected_error
goto exit_dbxtunetune
...
echo -----------------------------------------------------------------------
echo Must set the DBXTUNE_HOME variable to the place where you installed SqlServerTune.
echo -----------------------------------------------------------------------
goto exit_dbxtunetune
