...
if %JavaBitness% == 64 (
	...
	echo NOTE: Java is a 64 bit, RaxTune will be allowed to use more memory
)
...
java %JVM_MEMORY_PARAMS% %JVM_GC_PARAMS% %JVM_PARAMS% -Duser.language=en -Dsybase.home="%SYBASE%" -DSYBASE="%SYBASE%" -DAPPL_HOME="%SQLW_HOME%" -DDBXTUNE_HOME="%DBXTUNE_HOME%" -DDBXTUNE_SAVE_DIR="%DBXTUNE_SAVE_DIR%" %EXTRA% %DEBUG_OPTIONS% %SPLASH% com.asetune.RaxTune %*
...
echo -----------------------------------------------------------------------
echo Must set the DBXTUNE_HOME variable to the place where you installed RaxTune.
echo -----------------------------------------------------------------------
goto exit_dbxtune
...
echo Subject: RaxTune starting problem
echo -----------------------------------------------------------------------
echo .
goto exit_dbxtune
...
echo .
echo If you have a sql.ini file somewhere, RaxTune looks for it under %%SYBASE%%\ini\sql.ini.
