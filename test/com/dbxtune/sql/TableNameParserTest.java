...
public final class TableNameParserTest {
	...
	@Test
	public void testPgFunctionWithIntoVarAndFrom() {
		String sql = "" +
				"CREATE OR REPLACE FUNCTION public.t1_dummy2_func() \n" + 
				" RETURNS integer \n" + 
				" LANGUAGE plpgsql \n" + 
				"AS $function$ \n" + 
				"DECLARE \n" + 
				...
				"$function$ \n" + 
				"";
		assertThat(new TableNameParser(sql).tables(), equalTo(asSet("v_rowcount","t1")));
	}

	@Test
	public void testCallProc() {
		String sql = "call proc_name()";
		assertThat(new TableNameParser(sql).tables(), equalTo(asSet("proc_name")));
	}
