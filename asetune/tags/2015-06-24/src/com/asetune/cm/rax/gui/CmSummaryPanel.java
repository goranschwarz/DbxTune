...
import com.asetune.cm.rax.CmSummary;
...
//	private JScrollPane      _graphPanelScroll;
	...
	// from CmSummary: ra_status
	private JTextField       _raState_txt                  = new JTextField();
	private JLabel           _raState_lbl                  = new JLabel();
	private JTextField       _raAction_txt                 = new JTextField();
	private JLabel           _raAction_lbl                 = new JLabel();
	
	private JLabel           _ltlSentToRs_lbl              = new JLabel();
	private JTextField       _ltlSentToRs_Abs_txt          = new JTextField();
	private JTextField       _ltlSentToRs_Diff_txt         = new JTextField();
	private JTextField       _ltlSentToRs_Rate_txt         = new JTextField();

	private JLabel           _lrScanned_lbl                = new JLabel();
	private JTextField       _lrScanned_Abs_txt            = new JTextField();
	private JTextField       _lrScanned_Diff_txt           = new JTextField();
	private JTextField       _lrScanned_Rate_txt           = new JTextField();

	private JLabel           _lrTranProcessed_lbl          = new JLabel();
	private JTextField       _lrTranProcessed_Abs_txt      = new JTextField();
	private JTextField       _lrTranProcessed_Diff_txt     = new JTextField();
	private JTextField       _lrTranProcessed_Rate_txt     = new JTextField();
                                                           
	private JLabel           _ltlBytesSentToRs_lbl         = new JLabel();
	private JTextField       _ltlBytesSentToRs_Abs_txt     = new JTextField();
	private JTextField       _ltlBytesSentToRs_Diff_txt    = new JTextField();
	private JTextField       _ltlBytesSentToRs_Rate_txt    = new JTextField();

	private JLabel           _ltlKbSentToRs_lbl            = new JLabel();
	private JTextField       _ltlKbSentToRs_Abs_txt        = new JTextField();
	private JTextField       _ltlKbSentToRs_Diff_txt       = new JTextField();
	private JTextField       _ltlKbSentToRs_Rate_txt       = new JTextField();

	private JLabel           _ltlMbSentToRs_lbl            = new JLabel();
	private JTextField       _ltlMbSentToRs_Abs_txt        = new JTextField();
	private JTextField       _ltlMbSentToRs_Diff_txt       = new JTextField();
...
//		_localServerName_lbl  .setText("Local server name");
		...
		_listeners_lbl        .setText("RAX Port listeners");
		...
		_srvVersion_lbl       .setText("RAX Version");
		...
		_srvPageSize_lbl      .setText("ASE Page Size");
		...
		tooltip = "Values (State) from rs_status, if we are in ADMIN or REPLICATION mode...";
		_raState_lbl          .setText("RA In State");
		_raState_lbl          .setToolTipText(tooltip);
		_raState_txt          .setToolTipText(tooltip);
		_raState_txt          .setEditable(false);

		tooltip = "Values (Action) from rs_status, if we are in ADMIN or REPLICATION mode...";
		_raAction_lbl         .setText("RA State Action");
		_raAction_lbl         .setToolTipText(tooltip);
		_raAction_txt         .setToolTipText(tooltip);
		...
		_ltlSentToRs_Abs_txt  .setToolTipText(tooltip);
		_ltlSentToRs_Abs_txt  .setEditable(false);
		_ltlSentToRs_Diff_txt .setEditable(false);
		...
		_lrScanned_Abs_txt    .setToolTipText(tooltip);
		_lrScanned_Abs_txt    .setEditable(false);
		_lrScanned_Diff_txt   .setEditable(false);
		...
		_lrTranProcessed_Abs_txt  .setToolTipText(tooltip);
		_lrTranProcessed_Abs_txt  .setEditable(false);
		_lrTranProcessed_Diff_txt .setEditable(false);
		...
		_ltlBytesSentToRs_Abs_txt  .setToolTipText(tooltip);
		_ltlBytesSentToRs_Abs_txt  .setEditable(false);
		_ltlBytesSentToRs_Diff_txt .setEditable(false);
		...
		_ltlKbSentToRs_Abs_txt  .setToolTipText(tooltip);
		_ltlKbSentToRs_Abs_txt  .setEditable(false);
		_ltlKbSentToRs_Diff_txt .setEditable(false);
		...
		_ltlMbSentToRs_Abs_txt  .setToolTipText(tooltip);
		_ltlMbSentToRs_Abs_txt  .setEditable(false);
		_ltlMbSentToRs_Diff_txt .setEditable(false);
...
//		panel.add(_srvPageSize_txt,           "growx, wrap");
		...
		panel.add(_raState_lbl,               "gapy 20");
		panel.add(_raState_txt,               "growx, wrap");
                                              
		panel.add(_raAction_lbl,              "");
		panel.add(_raAction_txt,              "growx, wrap 20");
                                              
		                                      
		panel.add(_ltlSentToRs_lbl,           "hidemode 3");
		panel.add(_ltlSentToRs_Abs_txt,       "hidemode 3, growx, split");
		panel.add(_ltlSentToRs_Diff_txt,      "hidemode 3, growx, split");
		panel.add(_ltlSentToRs_Rate_txt,      "hidemode 3, growx, wrap");
		                                      
		panel.add(_lrScanned_lbl,             "hidemode 3");
		panel.add(_lrScanned_Abs_txt,         "hidemode 3, growx, split");
		panel.add(_lrScanned_Diff_txt,        "hidemode 3, growx, split");
		panel.add(_lrScanned_Rate_txt,        "hidemode 3, growx, wrap");
		                                      
		panel.add(_lrTranProcessed_lbl,       "hidemode 3");
		panel.add(_lrTranProcessed_Abs_txt,   "hidemode 3, growx, split");
		panel.add(_lrTranProcessed_Diff_txt,  "hidemode 3, growx, split");
		panel.add(_lrTranProcessed_Rate_txt,  "hidemode 3, growx, wrap");
		
		panel.add(_ltlBytesSentToRs_lbl,      "hidemode 3");
		panel.add(_ltlBytesSentToRs_Abs_txt,  "hidemode 3, growx, split");
		panel.add(_ltlBytesSentToRs_Diff_txt, "hidemode 3, growx, split");
		panel.add(_ltlBytesSentToRs_Rate_txt, "hidemode 3, growx, wrap");
		                                      
		panel.add(_ltlKbSentToRs_lbl,         "hidemode 3");
		panel.add(_ltlKbSentToRs_Abs_txt,     "hidemode 3, growx, split");
		panel.add(_ltlKbSentToRs_Diff_txt,    "hidemode 3, growx, split");
		panel.add(_ltlKbSentToRs_Rate_txt,    "hidemode 3, growx, wrap");
		                                      
		panel.add(_ltlMbSentToRs_lbl,         "hidemode 3");
		panel.add(_ltlMbSentToRs_Abs_txt,     "hidemode 3, growx, split");
		panel.add(_ltlMbSentToRs_Diff_txt,    "hidemode 3, growx, split");
		panel.add(_ltlMbSentToRs_Rate_txt,    "hidemode 3, growx, wrap");
...
//		panel.add(_totalErrorsDiff_txt,     "growx, wrap");
	...
	public void setComponentProperties()
	{
		...
		_ltlSentToRs_Abs_txt     .setVisible(showAbs); 
		_ltlSentToRs_Diff_txt    .setVisible(showDiff);
		_ltlSentToRs_Rate_txt    .setVisible(showRate);

		_lrScanned_Abs_txt       .setVisible(showAbs); 
		_lrScanned_Diff_txt      .setVisible(showDiff);
		_lrScanned_Rate_txt      .setVisible(showRate);

		_lrTranProcessed_Abs_txt .setVisible(showAbs); 
		_lrTranProcessed_Diff_txt.setVisible(showDiff);
		_lrTranProcessed_Rate_txt.setVisible(showRate);

		_ltlBytesSentToRs_Abs_txt .setVisible(showAbs); 
		_ltlBytesSentToRs_Diff_txt.setVisible(showDiff);
		_ltlBytesSentToRs_Rate_txt.setVisible(showRate);

		_ltlKbSentToRs_Abs_txt .setVisible(showAbs); 
		_ltlKbSentToRs_Diff_txt.setVisible(showDiff);
		_ltlKbSentToRs_Rate_txt.setVisible(showRate);

		_ltlMbSentToRs_Abs_txt .setVisible(showAbs); 
		_ltlMbSentToRs_Diff_txt.setVisible(showDiff);
		_ltlMbSentToRs_Rate_txt.setVisible(showRate);
...
//			setSummaryData(cm);
	...
	private void setFieldAbsDiffRate(CountersModel cm, String pkStr, String col, JLabel label, JTextField absField, JTextField diffField, JTextField rateField)
	{
		if (cm.isDataInitialized())
		{
			...
			if ( ! _originToolTip.containsKey(pkStr) )
				_originToolTip.put(pkStr, absField.getToolTipText());

			if (cm.findColumn(col) >= 0)
			{
//System.out.println("setFieldAbsDiffRate(): cm='"+cm.getName()+"', field='"+name+"', ABS="+cm.getAbsString (0, name)+", DIFF="+cm.getDiffString(0, name)+", RATE="+cm.getRateString(0, name)+".");
				...
				absField .setText(cm.getAbsString (pkStr, col));
				diffField.setText(cm.getDiffString(pkStr, col));
				rateField.setText(cm.getRateString(pkStr, col));
...
//System.out.println("setFieldAbsDiffRate(): cm='"+cm.getName()+"', field='"+name+"', hasNonConfiguredMonitoringHappened="+hasNonConfiguredMonitoringHappened);
				...
				{
					String tooltip = _originToolTip.get(pkStr);
					label    .setToolTipText(tooltip);
					absField .setToolTipText(tooltip);
					diffField.setToolTipText(tooltip);
					rateField.setToolTipText(tooltip);
...
//		_srvVersion_txt       .setText(cm.getAbsString (0, "atAtVersion").replaceFirst("Sybase IQ/", ""));  _srvVersion_txt.setCaretPosition(0);
		_raState_txt          .setText(cm.getAbsString (0, "State"));
		_raAction_txt         .setText(cm.getAbsString (0, "Action"));
...
//				System.out.println("summary: doPostRefresh(): Time statistics last reset:    " + cmRaStat.getAbsString("Time statistics last reset",    "Value") );

				_startDate_txt      .setText(   cmRaStat.getAbsString("Time replication last started", "Value")   );
				_countersCleared_txt.setText(   cmRaStat.getAbsString("Time statistics last reset",    "Value")   );
				
				
				setFieldAbsDiffRate(cmRaStat, "Number of LTL commands sent",  "NumberValue", _ltlSentToRs_lbl,      _ltlSentToRs_Abs_txt,      _ltlSentToRs_Diff_txt,      _ltlSentToRs_Rate_txt);
				setFieldAbsDiffRate(cmRaStat, "Total operations scanned",     "NumberValue", _lrScanned_lbl,        _lrScanned_Abs_txt,        _lrScanned_Diff_txt,        _lrScanned_Rate_txt);
				setFieldAbsDiffRate(cmRaStat, "Total transactions processed", "NumberValue", _lrTranProcessed_lbl,  _lrTranProcessed_Abs_txt,  _lrTranProcessed_Diff_txt,  _lrTranProcessed_Rate_txt);
				setFieldAbsDiffRate(cmRaStat, "Total bytes sent",             "NumberValue", _ltlBytesSentToRs_lbl, _ltlBytesSentToRs_Abs_txt, _ltlBytesSentToRs_Diff_txt, _ltlBytesSentToRs_Rate_txt);
				setFieldAbsDiffRate(cmRaStat, "Total bytes sent - KB",        "NumberValue", _ltlKbSentToRs_lbl,    _ltlKbSentToRs_Abs_txt,    _ltlKbSentToRs_Diff_txt,    _ltlKbSentToRs_Rate_txt);
				setFieldAbsDiffRate(cmRaStat, "Total bytes sent - MB",        "NumberValue", _ltlMbSentToRs_lbl,    _ltlMbSentToRs_Abs_txt,    _ltlMbSentToRs_Diff_txt,    _ltlMbSentToRs_Rate_txt);
			}
		}
