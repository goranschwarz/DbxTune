...
//		panel.add(createPcsPanel(),         "grow, hidemode 3");
	...
	private JPanel createAseServerPanel()
	{
		...
		_aseMonUrl_chk .setToolTipText("<html>Actual URL used to connect the monitored server.<br>"+urlExample+"</html>");
		_aseMonUrl_txt .setToolTipText("<html>Actual URL used to connect the monitored server.<br>"+urlExample+"</html>");
...
//		_ifile_txt.setEditable(false);
		...
		panel.add(_aseMonUrl_chk,     "");
		panel.add(_aseMonUrl_txt,     "push, grow");
		_aseMonUrl_txt.setEnabled(_aseMonUrl_chk.isEnabled());
		...
		_aseMonUrl_chk .addActionListener(this);
		_aseMonUrl_txt .addActionListener(this);
...
//		_aseOptionSavePwd_chk        .setToolTipText("Save the password in the configuration file, and yes it's encrypted");
		_aseOptionConnOnStart_chk    .setToolTipText("When AseMon starts use the above ASE and connect automatically (if the below 'Persisten Counter Storage' is enabled, it will also be used at startup)");
...
//		_hostmonPort_txt.addActionListener(this);
	...
	public final static String JDBC_URL_TOOLTIP = 
		...
		"<br>" +
		"For H2 you can use the following variables: <code>${DATE}, ${SERVERNAME}, ${ASEHOSTNAME}, ${ASEMON_HOME}, ${ASEMON_SAVE_DIR}</code><br>" +
		...
		"" +
		"Example:<br>" +
		"<code>${ASEMON_SAVE_DIR}/xxx.${ASEHOSTNAME}.${DATE:format=yyyy-MM-dd.HH;roll=true}</code><br>" +
		"<br>" +
		"The above example, does:" +
		"<ul>" +
		"  <li>H2 dbname would be '<i>asemonInstallDir</i>/data/xxx.host1.2011-05-31.21'. <br>" +
...
//		_pcsJdbcUrl_but   .addActionListener(this);
		...
		_pcsJdbcUrl_cbx   .addItem("jdbc:h2:file:${ASEMON_SAVE_DIR}/${ASEHOSTNAME}_${DATE}");
...
//			AseConnectionFactory.setPassword( _asePasswd_txt.getText() );
			...
			String rawUrl   = _aseMonUrl_txt.getText();
...
//			sendOpt.put("CHARSET", "wow... look at this");
		...
		if (_pcsJdbcUrl_but.equals(source))
		{
			...
			File baseDir = h2help.getDir(System.getProperty("ASEMON_SAVE_DIR"));
...
//				}
		...
		// --- ASE: URL Field ---
		if (_aseMonUrl_txt.equals(source))
		{
			try 
			{
				AseUrlHelper aseUrl = AseUrlHelper.parseUrl(_aseMonUrl_txt.getText());
...
//			}
	...
	*/
	...
	private void setUrlText()
	{
		if (_aseMonUrl_chk.isSelected())
		{
			_aseMonUrl_txt  .setEnabled(true);
		...
		_aseMonUrl_txt  .setEnabled(false);
	...
	*/	
	private void saveProps()
	{
		...
		conf.setProperty("conn.url.raw",                   _aseMonUrl_txt.getText() );
		conf.setProperty("conn.url.raw.checkbox",          _aseMonUrl_chk.isSelected() );
...
//		Configuration conf = Configuration.getInstance(Configuration.TEMP);
		...
		if (str != null)
			_aseMonUrl_txt.setText(str);

		bol = conf.getBooleanProperty("conn.url.raw.checkbox", false);
		_aseMonUrl_chk.setSelected(bol);
