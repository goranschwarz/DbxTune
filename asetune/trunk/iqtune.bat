...
set title=IqTune-console
title %title%

rem ------------------------------------------------------------------------
rem --- set IQTUNE_HOME to current directory if NOT already set
rem ------------------------------------------------------------------------
IF "%IQTUNE_HOME%"=="" set IQTUNE_HOME=%~dp0
...
rem ------------------------------------------------------------------------
rem --- remove last '\' char from: IQTUNE_HOME and SYBASE
rem --- IQTUNE_HOME exists every time, while SYBASE is NOT mandatory
rem --- Also for SYBASE
...
rem ------------------------------------------------------------------------
IF %IQTUNE_HOME:~-1%==\ SET IQTUNE_HOME=%IQTUNE_HOME:~0,-1%
...
rem ------------------------------------------------------------------------
set APPL_HOME=%IQTUNE_HOME%
set IQTUNE_SAVE_DIR=%IQTUNE_HOME%\data
...
IF DEFINED IQTUNE_JVM_MEMORY_PARAMS set JVM_MEMORY_PARAMS_32=%IQTUNE_JVM_MEMORY_PARAMS%
IF DEFINED IQTUNE_JVM_MEMORY_PARAMS set JVM_MEMORY_PARAMS_64=%IQTUNE_JVM_MEMORY_PARAMS%
...
IF DEFINED IQTUNE_JVM_GC_PARAMS set JVM_GC_PARAMS_32=%IQTUNE_JVM_GC_PARAMS%
IF DEFINED IQTUNE_JVM_GC_PARAMS set JVM_GC_PARAMS_64=%IQTUNE_JVM_GC_PARAMS%
...
IF NOT DEFINED IQTUNE_HOME GOTO no_iqtunehome
...
rem ------------------------------------------------------------------------
set classpath=%IQTUNE_HOME%\classes
set classpath=%classpath%;%IQTUNE_HOME%\lib\asetune.jar
...
set classpath=%classpath%;%IQTUNE_HOME%\lib\wizard.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\miglayout-swing-4.2.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\miglayout-core-4.2.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\swingx-all-1.6.5-1.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\jchart2d-3.2.2.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\planviewer.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\commons-cli-1.2.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\proxy-vole_20131209.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\ganymed-ssh2-build251beta1.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\rsyntaxtextarea.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\autocomplete.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\rstaui.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\jcommon-1.0.21.jar
set classpath=%classpath%;%IQTUNE_HOME%\lib\jfreechart-1.0.17.jar
...
rem ------------------------------------------------------------------------
set PATH=%IQTUNE_JAVA_HOME%\bin;%JAVA_HOME%\bin;%PATH%
...
java %JVM_MEMORY_PARAMS% %JVM_GC_PARAMS% %JVM_PARAMS% -Dsybase.home="%SYBASE%" -DSYBASE="%SYBASE%" -DAPPL_HOME="%SQLW_HOME%" -DIQTUNE_HOME="%IQTUNE_HOME%" -DIQTUNE_SAVE_DIR="%IQTUNE_SAVE_DIR%" %EXTRA% %DEBUG_OPTIONS% %SPLASH% com.asetune.IqTune %*
...
:no_iqtunehome
echo -----------------------------------------------------------------------
echo Error: no IQTUNE_HOME environment variable.
echo -----------------------------------------------------------------------
echo Must set the IQTUNE_HOME variable to the place where you installed IqTune.
echo -----------------------------------------------------------------------
goto exit_iqtune
...
echo Subject: IqTune starting problem
echo -----------------------------------------------------------------------
echo .
goto exit_iqtune
...
echo .
echo If you have a sql.ini file somewhere, IqTune looks for it under %%SYBASE%%\ini\sql.ini.
