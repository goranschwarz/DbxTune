 ...
 * Icons, that is needed to be created
	- com/asetune/images/db2tune_splash.jpg                - Showned at GUI startup
	- com/asetune/images/db2tune_icon_16.png               - application small icon
	- com/asetune/images/db2tune_icon_32.png               - application bigger icon
	- com/asetune/images/tcp_group_icon_db2.png            - icon at the "server" tab
 ...
 * build scripts (build.xml)
	...
	<!--<copy file="db2tune.sh"                                    toFile="${dist}/beta/db2tune.sh"/>-->
		<copy file="${src}/com/asetune/images/db2tune_splash.jpg"  toFile="${dist}/beta/lib/db2tune_splash.jpg"/>
 ...
 * change: src/com/asetune/DbxTune.java
	- in the main(), add Db2Tune
	  -> else if ("Db2Tune"      .equalsIgnoreCase(_mainClassName)) _instance = new Db2Tune      (cmd);
 
 * create: src/com/asetune/Db2Tune.java (take a copy of MySqlTune.java for instance))
	- change everuthing that says 'MySql' to 'Db2'
	- This requires you to create X number of new files, that implements DB2 Specifics
		src/com/asetune/check/CheckForUpdatesDb2.java
		src/com/asetune/CounterControllerDb2
		src/com/asetune/gui/MainFrameDb2.java
		[src/com/asetune/config/dict/MonTablesDictionaryDb2.java]
		[src/com/asetune/config/dbms/Db2Config.java]
		[src/com/asetune/config/dbms/Db2ConfigText.java]

Note: From here on it's a little "hacky" since we need to create a bunch of clases... that depends on other clases...
	
 * create: src/com/asetune/check/CheckForUpdatesDb2.java (take a copy from CheckForUpdatesMySql.java)
	- no changes needed (probably)
 
 * create: src/com/asetune/config/dict/MonTablesDictionaryDb2.java 
 ...
 * create: src/com/asetune/CounterControllerDb2
 ...
 * create the first CM (CmSummary)
	- create package: com/asetune/cm/db2 & com/asetune/cm/db2/gui
	- create: com/asetune/cm/db2/CmSummary.java				(note in import: dont point to the "wrong" com.asetune.cm.>>>db2<<<.gui.CmSummaryPanel)
		change getSqlForVersion() to reflect what you want to get in the summary (view other CmSummary.java for inspiration) 
	- create: com/asetune/cm/db2/gui/CmSummaryPanel.java    (note in import: dont point to the "wrong" com.asetune.cm.>>>db2<<<.CmSummary)
 ...
 * optional (if various components are DBMS Version dependant)
	- create: com.asetune.sql.conn.Db2Connection.java
	- edit: com.asetune.sql.conn.DbxConnection.java
