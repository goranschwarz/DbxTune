...
import com.asetune.cm.ase.CmRaSqlActivity;
import com.asetune.cm.ase.CmRaSqlMisses;
...
//			   )
	 ...
	 * this so we can decide what monitor tables and columns we could use.
	 */
	public void createCounters_OLD()
	{
		if (_countersIsCreated)
			return;
...
//		Configuration conf = Configuration.getInstance(Configuration.CONF);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmSummary.create(this, MainFrame.getInstance());
...
//
//		tmp = new CountersModel(name, null, null, // NO GROUP
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmObjectActivity.create(this, MainFrame.getInstance());
...
//        // 15.7.0.1     add    HkgcRequestsDcomp         int           Total number of data pages of the object that were queued in Housekeeper Garbage Collection queues for page compression
//        // 15.7.0.1     add    HkgcPendingDcomp          int           Number of data pages of the object that are still pending in Housekeeper Garbage Collection queues for page compression
//        // 15.7.0.1     add    HkgcOverflowsDcomp        int           Number of data pages of the object that overflowed due to lack of space in Housekeeper Garbage Collection queues for page compression
//        //---------------------------------------------------------------------------------------------------
//
...
//		colsCalcDiff = new String[] {"LogicalReads","PhysicalReads", "APFReads", "PagesRead", "PhysicalWrites", "PagesWritten", "UsedCount", "RowsInsUpdDel", "RowsInserted", "RowsDeleted", "RowsUpdated", "Operations", "LockRequests", "LockWaits", "HkgcRequests", "HkgcPending", "HkgcOverflows", "OptSelectCount", "PhysicalLocks", "PhysicalLocksRetained", "PhysicalLocksRetainWaited", "PhysicalLocksDeadlocks", "PhysicalLocksWaited", "PhysicalLocksPageTransfer", "TransferReqWaited", "TotalServiceRequests", "PhysicalLocksDowngraded", "PagesTransferred", "ClusterPageWrites", "SharedLockWaitTime", "ExclusiveLockWaitTime", "UpdateLockWaitTime", "HkgcRequestsDcomp", "HkgcOverflowsDcomp"};
...
//
//
//		tmp = new CountersModel(name, MainFrame.TCP_GROUP_OBJECT_ACCESS, 
//				null, pkList, colsCalcDiff, colsCalcPCT, 
//				monTables, needRole, needConfig, needVersion, needCeVersion, 
//				true, true)
//		{
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName())
//			{
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmProcessActivity.create(this, MainFrame.getInstance());
...
//		
//		
//		tmp = new CountersModel(name, MainFrame.TCP_GROUP_SERVER, 
//				null, pkList, colsCalcDiff, colsCalcPCT, 
//				monTables, needRole, needConfig, needVersion, needCeVersion, 
//				true, true)
//		{
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmSpidWait   .create(this, MainFrame.getInstance());
		CmSpidCpuWait.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmOpenDatabases.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmTempdbActivity.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmSysWaits.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmEngines.create(this, MainFrame.getInstance());
...
//		// 15.7.0.1     add    HkgcPendingItemsDcomp  int              Number of data pages yet to be page compressed by housekeeper garbage collector on this engine
//		// 15.7.0.1     add    HkgcOverflowsDcomp     int              Number of data pages that could not be queued to housekeeper garbage collector for data page compression due to queue overflows
//		//---------------------------------------------------------------------------------------------------
//		
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmSysLoad.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmDataCaches.create(this, MainFrame.getInstance());
...
//
//		tmp = new CountersModel(name, MainFrame.TCP_GROUP_CACHE, 
//				null, pkList, colsCalcDiff, colsCalcPCT, 
//				monTables, needRole, needConfig, needVersion, needCeVersion, 
//				true, true)
//		// OVERRIDE SOME DEFAULT METHODS
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmCachePools.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmDeviceIo.create(this, MainFrame.getInstance());
...
//
//		tmp = new CountersModel(name, MainFrame.TCP_GROUP_DISK, 
//				null, pkList, colsCalcDiff, colsCalcPCT, 
//				monTables, needRole, needConfig, needVersion, needCeVersion, 
//				true, true)
//		// OVERRIDE SOME DEFAULT METHODS
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmIoQueueSum.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmIoQueue.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmSpinlockSum.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmSysmon.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		// RepAgent: monRepSenders
		//-----------------------------------------
		//-----------------------------------------
		//-----------------------------------------
		CmRaSenders.create(this, MainFrame.getInstance());

//		name         = CM_NAME__RA_SENDERS;
//		displayName  = CM_DESC__RA_SENDERS;
//		description  = "<html>Provides information on Rep Agent Sender tasks.</html>";
//
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		needVersion  = 15700;
//		needCeVersion= 0;
//		monTables    = new String[] {"monRepSenders"};
//		needRole     = new String[] {"sa_role"};
//		needConfig   = new String[] {};
//		colsCalcDiff = new String[] {
//				"MessagesInQueue", 
...
//				};
//		colsCalcPCT  = new String[] {};
//		pkList       = null; // find out in: getPkForVersion()
//
//		tmp = new CountersModel(name, MainFrame.TCP_GROUP_REP_AGENT, 
//				null, pkList, colsCalcDiff, colsCalcPCT, 
//				monTables, needRole, needConfig, needVersion, needCeVersion, 
//				true, true, 0)
//		// OVERRIDE SOME DEFAULT METHODS
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName());
//			tcp.setToolTipText( description );
//			tcp.setIcon( SwingUtils.readImageIcon(Version.class, "images/cm_repagent_Senders.png") );
//			tcp.setCm(tmp);
//			MainFrame.addTcp( tcp );
//
//			tmp.setTabPanel( tcp );
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		// RepAgent: monRepLogActivity
		//-----------------------------------------
		//-----------------------------------------
		//-----------------------------------------
		CmRaLogActivity.create(this, MainFrame.getInstance());

//		name         = CM_NAME__RA_LOGACTIVITY;
//		displayName  = CM_DESC__RA_LOGACTIVITY;
//		description  = "<html>Provides information on Rep Agent Log Activity.</html>";
//
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		needVersion  = 15700;
//		needCeVersion= 0;
//		monTables    = new String[] {"monRepLogActivity"};
//		needRole     = new String[] {"sa_role"};
//		needConfig   = new String[] {};
//		colsCalcDiff = new String[] {
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName());
//			tcp.setToolTipText( description );
//			tcp.setIcon( SwingUtils.readImageIcon(Version.class, "images/cm_repagent_LogActivity.png") );
//			tcp.setCm(tmp);
//			MainFrame.addTcp( tcp );
//
//			tmp.setTabPanel( tcp );
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		// RepAgent: monRepScanners
		//-----------------------------------------
		//-----------------------------------------
		//-----------------------------------------
		CmRaScanners.create(this, MainFrame.getInstance());

//		name         = CM_NAME__RA_SCANNERS;
//		displayName  = CM_DESC__RA_SCANNERS;
//		description  = "<html>Provides information on Rep Agent Scanner tasks.</html>";
//
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		needVersion  = 15700;
//		needCeVersion= 0;
//		monTables    = new String[] {"monRepScanners"};
//		needRole     = new String[] {"sa_role"};
//		needConfig   = new String[] {};
//		colsCalcDiff = new String[] {"LogRecordsScanned", "LogRecordsProcessed", "NumberOfTruncPointRequested", "NumberOfTruncPointMoved"};
//		colsCalcPCT  = new String[] {};
//		pkList       = null; // find out in: getPkForVersion()
//
//		tmp = new CountersModel(name, MainFrame.TCP_GROUP_REP_AGENT, 
...
//			public String getSqlForVersion(Connection conn, int aseVersion, boolean isClusterEnabled)
//			{
//				String sql = "select * from master..monRepScanners";
//				return sql;
//			}
//		};
//
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName());
//			tcp.setToolTipText( description );
//			tcp.setIcon( SwingUtils.readImageIcon(Version.class, "images/cm_repagent_Scanners.png") );
//			tcp.setCm(tmp);
//			MainFrame.addTcp( tcp );
//
//			tmp.setTabPanel( tcp );
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		// RepAgent: monRepScannersTotalTime
		//-----------------------------------------
		//-----------------------------------------
		//-----------------------------------------
		CmRaScannersTime.create(this, MainFrame.getInstance());

//		name         = CM_NAME__RA_SCANNERS_TIME;
//		displayName  = CM_DESC__RA_SCANNERS_TIME;
//		description  = "<html>Provides information on the time spent by the Rep Agent Scanner tasks.</html>";
//
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		needVersion  = 15700;
//		needCeVersion= 0;
//		monTables    = new String[] {"monRepScannersTotalTime"};
//		needRole     = new String[] {"sa_role"};
//		needConfig   = new String[] {};
//		colsCalcDiff = new String[] {
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName());
//			tcp.setToolTipText( description );
//			tcp.setIcon( SwingUtils.readImageIcon(Version.class, "images/cm_repagent_ScannersTime.png") );
//			tcp.setCm(tmp);
//			MainFrame.addTcp( tcp );
//
//			tmp.setTabPanel( tcp );
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		// RepAgent: monSQLRepActivity
		//-----------------------------------------
		//-----------------------------------------
		//-----------------------------------------
		CmRaSqlActivity.create(this, MainFrame.getInstance());

//		name         = CM_NAME__RA_SQL_ACTIVITY;
//		displayName  = CM_DESC__RA_SQL_ACTIVITY;
//		description  = "<html>Provides statistics on statements replicated as SQL for all open objects.</html>";
//
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		needVersion  = 15030;
//		needCeVersion= 0;
//		monTables    = new String[] {"monSQLRepActivity"};
//		needRole     = new String[] {"sa_role"};
//		needConfig   = new String[] {};
//		colsCalcDiff = new String[] {"UpdateStmts", "InsertSelectStmts", "DeleteStmts", "SelectIntoStmts"};
//		colsCalcPCT  = new String[] {};
//		pkList       = null; // find out in: getPkForVersion()
//
//		tmp = new CountersModel(name, MainFrame.TCP_GROUP_REP_AGENT, 
...
//			public String getSqlForVersion(Connection conn, int aseVersion, boolean isClusterEnabled)
//			{
//				String sql = "select * from master..monSQLRepActivity";
//				return sql;
//			}
//		};
//
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName());
//			tcp.setToolTipText( description );
//			tcp.setIcon( SwingUtils.readImageIcon(Version.class, "images/cm_repagent_SqlActivity.png") );
//			tcp.setCm(tmp);
//			MainFrame.addTcp( tcp );
//
//			tmp.setTabPanel( tcp );
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		// RepAgent: monSQLRepMisses
		//-----------------------------------------
		//-----------------------------------------
		//-----------------------------------------
		CmRaSqlMisses.create(this, MainFrame.getInstance());

//		name         = CM_NAME__RA_SQL_MISSES;
//		displayName  = CM_DESC__RA_SQL_MISSES;
//		description  = "<html>Provides statistics on statements that could not be replicated as SQL for all open objects.</html>";
//
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		needVersion  = 15030;
//		needCeVersion= 0;
//		monTables    = new String[] {"monSQLRepMisses"};
//		needRole     = new String[] {"sa_role"};
//		needConfig   = new String[] {};
//		colsCalcDiff = new String[] {"Threshold", "QueryLimitation", "Configuration"};
//		colsCalcPCT  = new String[] {};
//		pkList       = null; // find out in: getPkForVersion()
//
//		tmp = new CountersModel(name, MainFrame.TCP_GROUP_REP_AGENT, 
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName());
//			tcp.setToolTipText( description );
//			tcp.setIcon( SwingUtils.readImageIcon(Version.class, "images/cm_repagent_SqlMisses.png") );
//			tcp.setCm(tmp);
//			MainFrame.addTcp( tcp );
//
//			tmp.setTabPanel( tcp );
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmRaSysmon.create(this, MainFrame.getInstance());

//		name         = CM_NAME__RA_SYSMON;
//		displayName  = CM_DESC__RA_SYSMON;
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName());
//			tcp.setToolTipText( description );
//			tcp.setIcon( SwingUtils.readImageIcon(Version.class, "images/cm_repagent_sysmon.png") );
//			tcp.setCm(tmp);
//			MainFrame.addTcp( tcp );
//
//			tmp.setTabPanel( tcp );
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmCachedProcs.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmProcCacheLoad.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmProcCallStack.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmCachedObjects.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmErrolog.create(this, MainFrame.getInstance());
...
//		
//		tmp = new CountersModelAppend( name, MainFrame.TCP_GROUP_SERVER, null, monTables, needRole, needConfig, needVersion, needCeVersion, true)
//		// OVERRIDE SOME DEFAULT METHODS
//		{
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmDeadlock.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmLockTimeout.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmPCacheModuleUsage.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmPCacheMemoryUsage.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmStatementCache.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmStmntCacheDetails.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmActiveObjects.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmActiveStatements.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmBlocking.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmTableCompression.create(this, MainFrame.getInstance());

//		//==================================================================================================
//		// 15.7.0: Description of: monTableCompression
//		// 
//		// Description:
//		// Contains the table’s compression history.
//		//
//		// Needs Configuration:
//		// Enable the "enable monitoring" and "per object statistics active" configuration parameters 
//		// for this monitoring table to collect data.
//		// 
//		// Name                           Datatype     Attributes  Ind Description
//		// ------------------------------ ------------ ----------- --- ------------------------------------------------------------
//		// InstanceID                     tinyint                    0 The Server Instance Identifier (cluster only)
//		// DBID                           int                        0 Database ID for the compressed table
//		// ObjectID                       int                        0 Object ID of the compressed table
//		// PartitionID                    int                        0 Partition ID of the compressed partition
//		// CompRowInserted                bigint                     1 Number of compressed rows inserted
//		// CompRowUpdated                 bigint                     1 Number of compressed rows updated
//		// CompRowForward                 bigint                     1 Number of compressed rows being forwarded because of updated
//		// CompRowScan                    bigint                     1 Number of compressed rows accessed
//		// RowDecompressed                bigint                     1 Number of rows being decompressed
//		// RowPageDecompressed            bigint                     1 Number of page-compressed rows being decompressed to be row-compressed
//		// ColDecompressed                bigint                     1 Number of columns being decompressed
//		// RowCompNoneed                  int                        1 Number of rows whose compressed row length exceeds its normal row length
//		// PageCompNoneed                 bigint                     1 Number of pages that are not suitable for page-level compress
//		// PagesCompressed                bigint                     1 Number of pages being page-level compressed
//		// AvgBytesSavedPageLevel         bigint                     1 The average number of bytes saved by page-level compression (B)
//		// TableName                      varchar(255)               0 Name of the compressed table
//		//---------------------------------------------------------------------------------------------------
//		// Column changes in various versions:
//		//
//		// Version      Action Name              Datatype   Attributes Description
...
//		//---------------------------------------------------------------------------------------------------
//		
//		name         = CM_NAME__TABLE_COMPRESSION;
//		displayName  = CM_DESC__TABLE_COMPRESSION;
//		description  = "<html>" +
...
//		needVersion  = 15700;
//		needCeVersion= 0;
//		monTables    = new String[] {"monTableCompression"};
//		needRole     = new String[] {"mon_role"};
//		needConfig   = new String[] {"enable monitoring=1", "per object statistics active=1", "capture compression statistics=1"};
//		colsCalcDiff = new String[] {"CompRowInserted", "CompRowUpdated", "CompRowForward", "CompRowScan", "RowDecompressed", "RowPageDecompressed", "ColDecompressed", "RowCompNoneed", "PageCompNoneed", "PagesCompressed"};
//		colsCalcPCT  = new String[] {};
//		pkList       = new LinkedList<String>(); // well this is done at getPkForVersion()
//		     pkList.add("InstanceID");
//		     pkList.add("DBID");
//		     pkList.add("ObjectID");
...
//			public String[] getDependsOnConfigForVersion(Connection conn, int srvVersion, boolean isClusterEnabled)
//			{
//				return new String[] {"enable monitoring=1", "per object statistics active=1", "capture compression statistics=1"};
//			}
//
//			@Override
//			public List<String> getPkForVersion(Connection conn, int aseVersion, boolean isClusterEnabled)
...
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp.getGroupName());
//			tcp.setToolTipText( description );
//			tcp.setIcon( SwingUtils.readImageIcon(Version.class, "images/cm_table_compression.png") );
//			tcp.setCm(tmp);
//			MainFrame.addTcp( tcp );
//
//			tmp.setTabPanel( tcp );
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmMissingStats.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmQpMetrics.create(this, MainFrame.getInstance());
...
//		{
//			final String localName = name;
//			TabularCntrPanel tcp = new TabularCntrPanel(tmp.getDisplayName(), tmp, tmp.getGroupName())
//			{
//				private static final long	serialVersionUID	= 1L;
//
//				@Override
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmSpMonitorConfig.create(this, MainFrame.getInstance());
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmOsIostat.create(this, MainFrame.getInstance());
...
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		tmp = new CounterModelHostMonitor(name, MainFrame.TCP_GROUP_HOST_MONITOR, CounterModelHostMonitor.HOSTMON_IOSTAT, null, true);
//
//		tmp.setDisplayName(displayName);
//		tmp.setDescription(description);
//		if (AseTune.hasGUI())
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmOsVmstat.create(this, MainFrame.getInstance());
...
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		tmp = new CounterModelHostMonitor(name, MainFrame.TCP_GROUP_HOST_MONITOR, CounterModelHostMonitor.HOSTMON_VMSTAT, null, true);
//
//		tmp.setDisplayName(displayName);
//		tmp.setDescription(description);
//		if (AseTune.hasGUI())
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmOsMpstat.create(this, MainFrame.getInstance());
...
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		tmp = new CounterModelHostMonitor(name, MainFrame.TCP_GROUP_HOST_MONITOR, CounterModelHostMonitor.HOSTMON_MPSTAT, null, true);
//
//		tmp.setDisplayName(displayName);
//		tmp.setDescription(description);
//		if (AseTune.hasGUI())
...
//		_CMList.add(tmp);
		...
		//-----------------------------------------
		//-----------------------------------------
		CmOsUptime.create(this, MainFrame.getInstance());
...
//		SplashWindow.drawProgress("Loading: Counter Model '"+name+"'");
//		
//		tmp = new CounterModelHostMonitor(name, MainFrame.TCP_GROUP_HOST_MONITOR, CounterModelHostMonitor.HOSTMON_UPTIME, null, true);
//
//		tmp.setDisplayName(displayName);
//		tmp.setDescription(description);
//		if (AseTune.hasGUI())
...
//		_CMList.add(tmp);
		...
		//#################################################
		// This is how I might do it in the future
		//#################################################
		GetCounters counterCollector = this;
		MainFrame   guiController    = MainFrame.getInstance();

		CmThreads   .create(counterCollector, guiController);
		CmWorkQueues.create(counterCollector, guiController);
		CmIoControllers.create(counterCollector, guiController);
