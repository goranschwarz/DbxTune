...
//		sb.append("<h2>Event ").append(id).append(": No writeup is available for this Event.</h2>");
	...
	private void init()
	{
		...
		desc   = new String[] {"Adaptive Server is attempting to write to a MASS (Memory Address Space Segment). A MASS is one or more contiguous pages Adaptive Server keeps in a data cache. However, in this event, the status of the MASS is \"changing,\" meaning another spid is updating the MASS. The spid initiating the write cannot write to the MASS until the MASS is no longer in use.",
		...
		desc   = new String[] {"When Adaptive Server issues an asynchronous prefetch (APF) on a page, another process is reading the MASS (Memory Address Space Segment) to which this page belongs. Adaptive Server must wait for the read to complete before continuing."};
		...
		desc   = new String[] {"A spid must make changes to a MASS (Memory Address Space Segment), but another spid is currently writing the MASS. The second spid must wait until the write completes."};
		...
		desc   = new String[] {"A spid attempts to make changes to the MASS (Memory Address Space Segment), but another spid is currently changing the MASS. The first spid must wait until the changes are complete before it can make changes to the MASS."};
		...
		desc   = new String[] {"A process writes a range of pages for an object to disk because of a change to the object, or because the object was removed from the metadata cache. However, another spid has an I/O outstanding on the MASS (Memory Address Space Segment), and the second process must sleep until the first process’s finish writing."};
		...
		desc   = new String[] {"A spid attempts to write to a MASS (Memory Address Space Segment), but another spid is already changing the MASS, so the first spid must wait until the changes are complete.",
		...
		desc   = new String[] {"When you run <CODE>dbcc checkstorage</CODE>, Adaptive Server determines whether it needs to perform a disk read to verify whether a fault exists in the disk copy of the MASS (Memory Address Space Segment). However, another process may have already started that read. Event 203 measures the time spent waiting for those reads to complete."};
