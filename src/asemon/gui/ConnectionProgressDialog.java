...
implements ActionListener, ConnectionProgressCallback
{
	...
	public static Connection connectWithProgressDialog(Window owner, String driver, String rawUrl, Properties props)
	throws Exception
	{
		_fixme_jdbcDriver = driver;
		_fixme_rawUrl     = rawUrl;
		_fixme_props      = props;
		return connectWithProgressDialog(owner, rawUrl);
	}
	public static Connection connectWithProgressDialog(Window owner, String urlStr)
	throws Exception
	{
		ConnectionProgressDialog cpd = null;
		if      (owner instanceof Dialog) cpd = new ConnectionProgressDialog((Dialog)owner, urlStr);
		else if (owner instanceof  Frame) cpd = new ConnectionProgressDialog( (Frame)owner, urlStr);
	...
	private ConnectionProgressDialog(Dialog owner, String urlStr)
	{
		super(owner, true);
		init(owner, urlStr);
	}
	private ConnectionProgressDialog(Frame owner, String urlStr)
	{
		super(owner, true);
		init(owner, urlStr);
	}
	private void init(Component owner, String urlStr)
	{
		_urlStr = urlStr;
		...
		{ 
			...
			if (extraTasks != null && extraTasks.length > 0 )
			{
				...
				System.out.println("ADJUSTED _hostPortArr: "+StringUtil.toCommaStr(_hostPortArr));
			}
...
//			System.out.println("ZZZZZZZZZZZZZZZZZZZZZZZZzzz");
			...
			try
			{
				Connection conn = ConnectionProgressDialog.connectWithProgressDialog(this, _hostPortUrl);
