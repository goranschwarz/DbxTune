...
//		panel.add( new MultiLineLabel(WIZ_HELP), WizardOffline.MigLayoutHelpConstraints );
		...
		Vector tabData = populatePcsTable();
...
//		_offlineJdbcUrl_cbx   .addItem("jdbc:sybase:Tds:<host>:<port>[/<dbname>][?OPT1=val,&OPT2=val]");
	...
	private Vector populatePcsTable()
	{
...
//			AseConnectionFactory.setPassword( _passwd_txt.getText() );
			...
			try
			{
				...
				_aseConn = ConnectionProgressDialog.connectWithProgressDialog(this, AseConnectionFactory.getDriver(), rawUrl, props);
...
//		AseConnectionFactory.setPort    ( port );
		...
		{
			AseConnectionFactory.setProperties(null);
		}
		else
		{
			Map options = StringUtil.parseCommaStrToMap(_options_txt.getText());
		...
		try
		{
			...
			_aseConn = ConnectionProgressDialog.connectWithProgressDialog(this, urlStr);
...
//				if ( System.getProperty("os.name").startsWith("Windows"))
		...
		{
			Map sendOpt = StringUtil.parseCommaStrToMap(_options_txt.getText());
//			sendOpt.put("OPT1", "val 1");
//			sendOpt.put("OPT2", "val 2");
//			sendOpt.put("CHARSET", "wow... look at this");
			
			Map outOpt = JdbcOptionsDialog.showDialog(this, AseConnectionFactory.getDriver(), AseConnectionFactory.getUrlTemplate(), sendOpt);
			// null == CANCEL
			if (outOpt != null)
...
//			}
	...
	*/
	...
	private void setUrlText()
	{
		...
		Map optionsMap  = StringUtil.parseCommaStrToMap(_options_txt.getText());
