...
//		_logger.setLevel(Level.TRACE);
	...
	public void check()
	{
		...
		try
		{
			_url = new URL(ASEMON_CHECK_UPDATE_URL);
		}
...
//		add("email", "goran_schwarz@hotmail.com");
	...
	public void sendConnectInfo()
	{
		...
		try
		{
			_url = new URL(ASEMON_CONNECT_INFO_URL);
		}
		catch (MalformedURLException ex)
		{ 
			// shouldn't happen
...
//			_checkSucceed = true;
	...
	public void sendUdcInfo()
	{
		...
		try
		{
			_url = new URL(ASEMON_UDC_INFO_URL);
		}
		catch (MalformedURLException ex)
		{ 
			// shouldn't happen
		...
		if (udcRows == 0)
		{
			_logger.debug("No 'udc.*' was was found in Configuration, skipping this.");
			return;
		}
...
//		checkThread.start();
	...
	public void sendCounterUsageInfo()
	{
		...
		try
		{
			_url = new URL(ASEMON_COUNTER_USAGE_INFO_URL);
		}
		catch (MalformedURLException ex)
		{ 
			// shouldn't happen
		...
		int rows = 0;
//		List<CountersModel> cmList = GetCounters.getCmList();
