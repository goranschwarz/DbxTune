...
extends AbstractTableModel
{
	...
	private   Vector        _colSqlTypeName = null;
...
//				if (counter >= 10 )
				...
				_logger.warn(_name+": getValueAt(row="+row+", col="+col+"): _rows.size()="+_rows.size()+", ArrayIndexOutOfBoundsException, retry counter="+counter+"... "+e.getMessage());
				//Thread.yield();
				try {Thread.sleep(3);}
			...
			catch (NullPointerException e)
			{
				...
				// the Vector at the same time as AWT-EventQueue thread refreshes 
				// filters/cellRendering or does something...
				_logger.warn(_name+": getValueAt(row="+row+", col="+col+"): NullPointerException, retry counter="+counter+"... "+e.getMessage());
				//Thread.yield();
				try {Thread.sleep(3);}
				catch (InterruptedException ignore) {}
			}
	...
	public void setValueAt(Object value, int row, int col)
	{
		...
		if ( ! (row < _rows.size()) )
			for (int i=_rows.size(); i<=row; i++)
				_rows.add(new Vector(_nbCols));

		// Expand cols to correct numbers, if there wasn't enough
		Vector r = (Vector)_rows.get(row);
		if ( ! (col < r.size()) )
...
//	}
	...
	*/
	...
	public void setColumnNames(Vector cols)
	{
		_colNames = cols;
		_nbCols = _colNames.size();
	}

	public void setColumnNames(List cols)
	{
		_colNames = new Vector(cols);
		_nbCols = _colNames.size();
	}
	...
	{
		return (String)_colSqlTypeName.get(colId);
	}
		...
		for (int rowId=0; rowId < diff.getRowCount(); rowId++) 
		{
			...
			for (int i=0; i<diff.getColumnCount(); i++) 
			{
				...
				else if ( isDiffCol[i] ) 
				{
					...
					// IF String, skip the calculation
					if ( originObject instanceof String )
					{
						String colName = diff.getColumnName(i);
						_logger.warn("String columns can't be 'rate' calculated. colName='"+colName+"', originObject='"+originObject+"', keeping this object..");
			...
			if (oldRowId != -1)
			{
				...
				for (int i = 0; i < newSample.getColumnCount(); i++)
				{
					...
					else
					{
						...
						if ( newRowObj instanceof String)
						{
							String colName = newSample.getColumnName(i);
							_logger.warn("String columns can't be 'diff' calculated. colName='"+colName+"', key='"+key+"', oldObj='"+oldRowObj+"', newObj='"+newRowObj+"'.");
							diffRow.add(newRowObj);
						}
						else if ( newRowObj instanceof BigDecimal || newRowObj instanceof Long )
						{
						...
						else if ( newRowObj instanceof Float)
						{
							float prevSample = ((Float)oldRowObj).floatValue();
							float thisSample = ((Float)newRowObj).floatValue();
							float diff = thisSample - prevSample;
							//System.out.println("BigDecimal: i="+i+", diff="+diff+", prevSample="+prevSample+", thisSample="+thisSample);
							if (diff < 0)
							{
								if (diffCnt._negativeDiffCountersToZero)
