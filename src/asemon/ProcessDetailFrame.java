...
//public    JDialog          processLocksFrame            = null;
	...
	public ProcessDetailFrame(int k)
	{
//		pdf = this;
		try
		{
	...
	throws Exception
	{
		...
		saveButton.setToolTipText("Save Captured Statements Table to file ('capStmts.bcp', 'capStmts.txt', 'capStmts.ddl.sql' in ASEMON_SAVE_DIR or ASEMON_HOME).");
...
//		cnx = OpenConnectionDlg.getAnotherConnection(Version.getAppName()+"-spid", true);
		cnx = AseConnectionFactory.getConnection(null, Version.getAppName()+"-spid");
	...
	private void saveProps()
  	{
		Configuration asemonProps = Asemon.getSaveProps();
  	...
  	private void loadProps()
  	{
		...
		Configuration props = Asemon.getSaveProps();
		String base = "processDetailFrame.spid.";

		if (props == null)
			return;

		width  = props.getIntProperty(base + "window.width",  -1);
		height = props.getIntProperty(base + "window.height", -1);
		...
		width  = props.getIntProperty(base + "currentStatements.width",  -1);
		height = props.getIntProperty(base + "currentStatements.height", -1);
		if (width != -1 && height != -1)
		{
			currentStatementsPan.setPreferredSize(new Dimension(width, height));
		}

		width  = props.getIntProperty(base + "capturedStatements.width",  -1);
		height = props.getIntProperty(base + "capturedStatements.height", -1);
		if (width != -1 && height != -1)
		{
