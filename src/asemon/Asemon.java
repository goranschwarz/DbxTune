...
//		_cmdLine = cmd;
		...
		final String USER_CONFIG_FILE_NAME = System.getProperty("USER_CONFIG_FILE_NAME", "asemon.user.properties");
		...
		//System.out.println("AseMon has GUI="+_gui);
		...
		else
		{
			...
			if (cmd.hasOption('d'))
			{
				...
				else if (opt_D.equals("ASE"))
				{
					...
					String jdbcUrl    = "jdbc:sybase:Tds:"+aseHost+":"+asePort+"/"+aseDbname;
...
//		_logger.info("Using Java VM Specification Name:    "+System.getProperty("java.vm.specification.name"));
		...
		if (Version.IS_DEVELOPMENT_VERSION)
		{
			...
			if ( System.currentTimeMillis() > Version.DEV_VERSION_EXPIRE_DATE.getTime() )
			{
				...
				if (_gui)
				{
					SwingUtils.showErrorMessage("AseMon - This DEVELOPMENT VERSION has expired", 
							...
							"should only be used if the new AseMon version is not capable of reading the " +
							"old database. In 99.9% the new AseMon version will be able to read " +
							"Performance Counters stored by a earlier AseMon release.",
							ex);
				}
				else
				{
		...
		else
		{
			...
			{
				_logger.info("Installing a Swing EDT (Event Dispatch Thread) - Hang Monitor, which will write information about long running EDT operations to the AseMon log.");
...
//	}
	...
	private static void printHelp(Options options, String errorStr)
	{
		...
		pw.println("              [-n <cfgFile|cmNames>] [-l] [-i <seconds>] ");
		...
		pw.println("  -l                        List cm's that can be used in '-n' switch.");
...
//		}
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			//-------------------------------
			// Start ASEMON, GUI/NOGUI will be determined later on.
			//-------------------------------
			else
			{
				new Asemon(cmd);
