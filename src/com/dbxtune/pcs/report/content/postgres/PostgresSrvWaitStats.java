...
import com.dbxtune.config.dict.PostgresWaitTypeDictionary;
...
//	}
	...
	throws IOException
	{
		w.append("Wait Statistics for the full day (origin: CmPgSrvWait / pg_wait_sampling_profile) <br>");
		w.append("<br>");
...
//		sb.append("<br>");
		...
		w.append("<br>");
		w.append("Postgres Source table is 'pg_wait_sampling_profile'. <br>");
		w.append("PCS Source table is 'CmPgSrvWait_diff'. (PCS = Persistent Counter Store) <br>");
		...
		w.append(getDbxCentralLinkWithDescForGraphs(false, "Below are Wait Statistics on the Server level, what is the server waiting for.",
				"CmPgSrvWait_SrvWaitTypeMs",
				"CmPgSrvWait_SrvWaitMs"
				));

		_CmPgSrvWait_byClass .writeHtmlContent(w, null, null);
		_CmPgSrvWait_byEvent .writeHtmlContent(w, null, null);
	}

	@Override
	public String getSubject()
	{
		return "Wait Statistics for the full day (origin: CmPgSrvWait / pg_wait_sampling_profile)";
	}
	...
	public static final String PROPKEY_SamplePeriodInMinutes = "DailySummaryReport.PostgresSrvWaitStats.samplePeriodInminutes";
	public static final int    DEFAULT_SamplePeriodInMinutes = 10;

	public static final String PROPKEY_TopCount              = "DailySummaryReport.PostgresSrvWaitStats.top.count";
	public static final int    DEFAULT_TopCount              = 20;

//	public static final String PROPKEY_Selected              = "DailySummaryReport.PostgresSrvWaitStats.selected.wait_types";
//	public static final String DEFAULT_Selected              = "ASYNC_NETWORK_IO, LCK_M_IX, LCK_M_X, LATCH_SH, LATCH_EX, PAGEIOLATCH_SH, PAGEIOLATCH_EX, SOS_SCHEDULER_YIELD, ASYNC_IO_COMPLETION, WRITELOG, TDC, OLEDB";
	...
	{
		return new String[] { "CmPgSrvWait_diff" };
	}
...
//		_CmPgSrvWait_byClass     = createTsStackedBarChart(conn, "CmPgSrvWait", "WaitClassDesc", s, null, "WaitTime", null, null, "Wait Classes in seconds, grouped by 10 minutes intervall");
//		_CmPgSrvWait_byEvent     = createTsStackedBarChart(conn, "CmPgSrvWait", "WaitEventDesc", s, null, "WaitTime", null, null, "Wait Events  in seconds, grouped by 10 minutes intervall");
		...
		// High level -- CLASS
		//----------------------------------------
		_CmPgSrvWait_byClass = new ReportChartTimeSeriesStackedBar(this, conn, schema, "CmPgSrvWait", "event_type", samplePeriod, null, "WaitTimeInSec", null, null, null, "Wait Classes in Seconds, grouped by " + samplePeriod + " minutes intervall")
		{
			@Override
			protected String getSql()
			{
				...
				String sql = ""
					    ...
					    + "    ,[event_type] \n"
					    + "    ,CAST(sum([est_wait_time_ms])/1000 as BIGINT) AS [WaitTimeInSec] \n"
					    + "from [CmPgSrvWait_diff] \n"
					    + "where 1=1 \n"
						+ getReportPeriodSqlWhere()
//					    + "  and [WaitEventID] not in (178, 250) \n"
//					    + "  and [WaitClassDesc] not in ('waiting for internal system event') \n"
					    + "  and [est_wait_time_ms] > 0 \n"
					    + "group by DATEADD(MINUTE, DATEDIFF(MINUTE, '2000-01-01', [SessionSampleTime]) / " + samplePeriod + " * " + samplePeriod + ", '2000-01-01'), [event_type] \n"
					    + "order by 1, 2 \n"
					    + "";
				return sql;
			}
...
//				sb.append("  <th>SAP Wiki</th> \n");
				sb.append("  <th>Event Description</th> \n");
				sb.append("</tr> \n");
...
//					String waitEventId = StringUtils.substringBetween(entry.getKey(), "[", "]");
					String event_type = StringUtils.substringBefore(entry.getKey(), "::");
					String event      = StringUtils.substringAfter (entry.getKey(), "::");
...
//					String sparklineDataStr_wtpc = "";
					...
					//---- WaitTime -------------------------------------
					{
						...
						String sparklineClassName      = "sparklines_pgWaitTime__chart";
...
//									AggType.SUM,                                        // params.getGroupDataAggregationType()
									AggType.USER_PROVIDED,                              // params.getGroupDataAggregationType()
									null,                                               // params.getDbmsSchemaName()
									"CmPgSrvWait_diff",                                  // params.getDbmsTableName()
									"SessionSampleTime",                                // params.getDbmsSampleTimeColumnName()
//									"est_wait_time_ms",                                 // params.getDbmsDataValueColumnName()
//									false,                                              // params.getDbmsDataValueColumnNameIsExpression()
									"CAST(sum([est_wait_time_ms])/1000 as BIGINT)",     // params.getDbmsDataValueColumnName()
									true,                                               // params.getDbmsDataValueColumnNameIsExpression()
									Arrays.asList("event_type", "event"),               // colNameList
									Arrays.asList( event_type,   event),                // colValList
...
//							sparklineDataStr_wt = "<div class='" + sparklineClassName + "' values='" + valStr + "'>" + jfreeChartInlinePng + "</div>";
							...
							if (StringUtil.isNullOrBlank(sparklineJavaScriptInitCode_wt))
								sparklineJavaScriptInitCode_wt = SparklineHelper.getJavaScriptInitCode(PostgresSrvWaitStats.this, result, sparklineClassName, sparklineTooltipPostfix);
						}
...
//								sparklineJavaScriptInitCode_wtpc = SparklineHelper.getJavaScriptInitCode(PostgresSrvWaitStats.this, result, sparklineClassName, sparklineTooltipPostfix);
//						}
//						catch (SQLException ex)
//						{
...
//					sb.append("  <td>").append(entry.getKey()     ).append("</td> \n");
					sb.append("  <td>").append(event_type         ).append("</td> \n");
					sb.append("  <td>").append(event              ).append("</td> \n");
...
//					sb.append("  <td> <a href='https://wiki.scn.sap.com/wiki/display/SYBASE/ASE+Wait+Event+" + waitEventId + "' target='_blank'>WaitEventID - " + waitEventId + "</a>").append("</td> \n");
					sb.append("  <td>").append( PostgresWaitTypeDictionary.getWaitEventDescriptionPlain(event, false)).append("</td> \n");
					sb.append("</tr> \n");
				}
				sb.append("</table> \n");
				sb.append("<br> \n");
...
//						+ "having CAST(sum([est_wait_time_ms])/1000 as BIGINT) > 0 \n" // having [WaitTimeInSec] above 0
			...
			throws SQLException
			{
				String event_type = rs.getString(1);
				String event      = rs.getString(2);
				double value      = rs.getDouble(3);

				return new TopGroupDataRecord(event_type + "::" + event, value);
			}
		};
		...
		_CmPgSrvWait_byEvent = new ReportChartTimeSeriesStackedBar(this, conn, schema, "CmPgSrvWait", "event_type", samplePeriod, tgcp, "WaitTimeInSec", null, null, null, "Wait Events (top-"+tgcp.getTopCount()+"-WaitEvents) in Seconds, grouped by 10 minutes intervall")
		{
			...
			protected String getSql()
			{
				...
				String sql = ""
					    ...
					    + "  and [est_wait_time_ms] > 0 \n"
					    + "group by DATEADD(MINUTE, DATEDIFF(MINUTE, '2000-01-01', [SessionSampleTime]) / " + samplePeriod + " * " + samplePeriod + ", '2000-01-01'), [event_type], [event] \n"
//						+ "having CAST(sum([est_wait_time_ms])/1000 as BIGINT) > 0 \n" // having [WaitTimeInSec] above 0
					    + "order by 1, 2 \n"
					    + "";
				return sql;
			...
			public DataRecord createRecord(ResultSet rs) throws SQLException
			{
				...
				return new DataRecord(ts, event_type + "::" + event, value);
			}
		};
	}

	private IReportChart _CmPgSrvWait_byClass;
	private IReportChart _CmPgSrvWait_byEvent;
