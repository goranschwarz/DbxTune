...
//		Configurator.setRootLevel(Level.TRACE);
		...
		System.out.println("PG-FDW-1: " + test("FETCH 100 FROM c22"));

		// This grabs [clock_timestamp, xact_start, -1, query_start], while it should really just take [pg_stat_activity]
		// This is due to 'EXTRACT(... FROM xxx)' where we grab 'xxx' as a table name... could probably do workaround, but for now it's probably OK to grab "some" extra stuff...
		System.out.println("PG-xxx: " + test("select  datid ,datname ,pid ,CAST(state            as varchar(128)) AS state ,waiting ,usesysid ,usename ,CAST(application_name as varchar(128)) AS application_name ,CAST(false as boolean)                 AS has_sql_text ,CAST(false as boolean)                 AS has_pid_lock_info ,CAST(-1    as integer)                 AS pid_lock_count ,CAST(-1    as integer)                 AS pid_exlock_count ,CAST(false as boolean)                 AS has_blocked_pids_info ,CAST( COALESCE( EXTRACT('epoch' FROM clock_timestamp()) - EXTRACT('epoch' FROM xact_start ), -1) as numeric(12,1))  AS xact_start_sec ,CAST( COALESCE( EXTRACT('epoch' FROM clock_timestamp()) - EXTRACT('epoch' FROM query_start), -1) as numeric(12,1))  AS stmnt_start_sec ,CAST( CASE WHEN state = 'active'             THEN COALESCE( EXTRACT('epoch' FROM clock_timestamp()) - EXTRACT('epoch' FROM query_start ), -1) /* active -- query-elapsed-time */             ELSE COALESCE( EXTRACT('epoch' FROM state_change     ) - EXTRACT('epoch' FROM query_start ), -1) /* else   -- last-exec-time*/  END as numeric(12,1))                                                                                               AS stmnt_last_exec_sec ,CAST( COALESCE( EXTRACT('epoch' FROM clock_timestamp()) - EXTRACT('epoch' FROM state_change), -1) as numeric(12,1)) AS in_current_state_sec ,CAST( clock_timestamp() - xact_start   as varchar(30)) AS xact_start_age ,CAST( clock_timestamp() - query_start  as varchar(30)) AS stmnt_start_age ,CAST( CASE WHEN state = 'active'             THEN clock_timestamp() - query_start /* active -- query-elapsed-time */             ELSE state_change      - query_start /* else   -- last-exec-time*/        END as varchar(30))                              AS stmnt_last_exec_age ,CAST( clock_timestamp() - state_change as varchar(30)) AS in_current_state_age ,backend_start ,xact_start ,query_start ,state_change ,backend_xid ,backend_xmin ,CAST(client_addr      as varchar(128)) AS client_addr ,CAST(client_hostname  as varchar(128)) AS client_hostname ,client_port ,query                                  AS last_known_sql_statement ,CAST(null as text)                     AS pid_lock_info ,CAST(null as text)                     AS blocked_pids_info ,CASE WHEN state != 'active' OR state IS NULL THEN -1       ELSE cast(((EXTRACT('epoch' from CLOCK_TIMESTAMP()) - EXTRACT('epoch' from query_start)) * 1000) as int)  END as \"execTimeInMs\" ,CASE WHEN xact_start IS NULL THEN -1       ELSE cast(((EXTRACT('epoch' from CLOCK_TIMESTAMP()) - EXTRACT('epoch' from xact_start)) * 1000) as int)  END as \"xactTimeInMs\" from pg_stat_activity where state != 'idle'   and pid   != pg_backend_pid()   and application_name != 'PostgresTune' "));
	}
}
