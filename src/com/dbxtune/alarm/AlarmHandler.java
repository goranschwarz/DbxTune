...
import com.dbxtune.alarm.writers.AlarmWriterToApplicationLog;
...
//	public static final String  DEFAULT_WriterClass           = "com.dbxtune.alarm.writers.AlarmWriterToStdout";
...
//	private AlarmContainer _delayedAlarms     = new AlarmContainer();
	...
	*/
	...
	throws Exception
	{
		if (StringUtil.isNullOrBlank(writerClassName))
			throw new RuntimeException("addAlarmWriter(): writerClassName='" + writerClassName + "' is null or empty. This is not expected.");
		...
		if (writerClassName.startsWith("com.asetune."))
		{
			...
			_logger.warn("You passed an 'AlarmHandler' that starts with 'com.asetune...' [" + oldName + "], instead lets use the prefix 'com.dbxtune...' [" + newName + "].");
		}
...
//			srvName = "SERVER_NAME";
	...
	public void checkSendDummyAlarm(String srvName)
	{
		...
		if (probeFile1.exists())
		{
			...
			AlarmEvent dummyAlarm = new com.dbxtune.alarm.events.AlarmEventDummy(srvName, "SomeCmName", "SomeExtraInfo", Category.OTHER, Severity.INFO, ServiceState.UP, -1, 999, "Dummy alarm, just to test if the alarm handler is working", "Extended Description goes here", 0);
