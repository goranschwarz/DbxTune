...
import com.dbxtune.gui.DbSelectionForGraphsDialog;
...
//			LastLogBackupDurationDhms = "LastLogBackupDurationDhms = CASE WHEN od.LastTranLogDumpTime IS NULL OR od.LastTranLogEndDumpTime IS NULL OR od.LastTranLogEndDumpTime < od.LastTranLogDumpTime THEN NULL ELSE convert(varchar(4), datediff(day, od.LastTranLogDumpTime, od.LastTranLogEndDumpTime)) + 'd ' + convert(varchar(10), dateadd(second, datediff(second, od.LastTranLogDumpTime, od.LastTranLogEndDumpTime), '2000-01-01'), 108) END, \n";
			LastDbBackupDurationDhms  = "LastDbBackupDurationDhms  = CASE WHEN od.BackupStartTime     IS NULL OR od.BackupEndTime          IS NULL OR od.BackupEndTime          < od.BackupStartTime     THEN NULL ELSE convert(varchar(4), datediff(second, od.BackupStartTime,     od.BackupEndTime)          / 86400) + 'd ' + convert(varchar(10), dateadd(second, datediff(second, od.BackupStartTime,     od.BackupEndTime), '2000-01-01'), 108) END, \n";
			LastLogBackupDurationDhms = "LastLogBackupDurationDhms = CASE WHEN od.LastTranLogDumpTime IS NULL OR od.LastTranLogEndDumpTime IS NULL OR od.LastTranLogEndDumpTime < od.LastTranLogDumpTime THEN NULL ELSE convert(varchar(4), datediff(second, od.LastTranLogDumpTime, od.LastTranLogEndDumpTime) / 86400) + 'd ' + convert(varchar(10), dateadd(second, datediff(second, od.LastTranLogDumpTime, od.LastTranLogEndDumpTime), '2000-01-01'), 108) END, \n";
		}
...
//	private Map<String, Number> _map_alarm_LowLogFreeSpaceInPct = new HashMap<>();
	...
	public static final String  DEFAULT_alarm_LastLogBackupAgeInHoursSkipDbs  = "(master|model|tempdb)";
