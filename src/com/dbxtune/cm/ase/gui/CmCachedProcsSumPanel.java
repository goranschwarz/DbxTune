...
public class CmCachedProcsSumPanel
extends TabularCntrPanel
{
	...
	//	private static final Logger  _logger	           = Logger.getLogger(CmCachedProcsSumPanel.class);
	private static final long    serialVersionUID      = 1L;

	public CmCachedProcsSumPanel(CountersModel cm)
	{
		super(cm);
...
//		JPanel panel = SwingUtils.createPanel("Local Options", true);
		...
		l_sample_statementCacheObjects_chk = new JCheckBox("Sample Lightweight Procs for Statement Cache",        conf == null ? CmCachedProcsSum.DEFAULT_sample_statementCacheObjects : conf.getBooleanProperty(CmCachedProcsSum.PROPKEY_sample_statementCacheObjects, CmCachedProcsSum.DEFAULT_sample_statementCacheObjects));
		l_sample_dynamicSqlObjects_chk     = new JCheckBox("Sample Lightweight Procs for Dynamic SQL Statements", conf == null ? CmCachedProcsSum.DEFAULT_sample_dynamicSqlObjects     : conf.getBooleanProperty(CmCachedProcsSum.PROPKEY_sample_dynamicSqlObjects,     CmCachedProcsSum.DEFAULT_sample_dynamicSqlObjects));

		l_sample_statementCacheObjects_chk.setName(CmCachedProcsSum.PROPKEY_sample_statementCacheObjects);
		l_sample_dynamicSqlObjects_chk    .setName(CmCachedProcsSum.PROPKEY_sample_dynamicSqlObjects);
		...
		l_sample_statementCacheObjects_chk.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if (conf == null) return;
				conf.setProperty(CmCachedProcsSum.PROPKEY_sample_statementCacheObjects, ((JCheckBox)e.getSource()).isSelected());
				conf.save();
		...
		l_sample_dynamicSqlObjects_chk.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				Configuration conf = Configuration.getInstance(Configuration.USER_TEMP);
				if (conf == null) return;
				conf.setProperty(CmCachedProcsSum.PROPKEY_sample_dynamicSqlObjects, ((JCheckBox)e.getSource()).isSelected());
				conf.save();
