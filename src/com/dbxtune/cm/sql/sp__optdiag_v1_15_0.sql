...
-- the field 'SP__OPTDIAG_CR_STR' in 'com.dbxtune.cm.sql.VersionInfo'
--
...
create procedure sp__optdiag
 ...
 , @proc_version varchar(78) = 'sp__optdiag/1.15.0/0/B/KJS/AnyPlat/AnyOS/12.5.x/Tue Jun 20 11:28:10 2006'
...
**
**	Author:  Kevin Sherlock - original version.  email:kevin.sherlock@teamsybase.com
...
**      - This stored procedure has been tested on ASE versions 11.9.2 through 15.0 ESD#2, but only
**        on HP-UX, Solaris SPARC, Linux x86, and Windows.
...
**
**               06/20/2006 (ksherlock) 1.15.0
**                  - Compatibility with ASE 15.0.2
**                  - Removed 'spare1' column and replaced with 'plljoindegree'
**                  - Note:  Output display is 12.5.x style and currently doesn't include 15
**                           display info such as partition info specific to 15.
**
*************************************************************************************************/
...
/*
   ...
   where id = @tabid
*/
   select @ptn_data_pgs = convert(int, max(data_pages(@u_dbid,@tabid,0,partitionid)))
   from syspartitions
   where id = @tabid
   ...
   begin
      ...
      select @leafcnt = ltrim(convert(varchar(32),convert(int,i.leafcnt))),
             ...
             @plljoindegree = ltrim(convert(varchar(32),convert(int,i.plljoindegree))),
