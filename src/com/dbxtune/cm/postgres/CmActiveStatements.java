...
////		setColumnShorterLength(rsmdc, "backend_xid"      , 20);  // xid  --- Already set to varchar(30) in com.dbxtune.sql.ddl.DbmsDdlResolverPostgres
////		setColumnShorterLength(rsmdc, "backend_xmin"     , 20);  // xid  --- Already set to varchar(30) in com.dbxtune.sql.ddl.DbmsDdlResolverPostgres
//		
//		// In PG 9.6
...
// // DbxCentral "buttons" -- add "template" so we can label the buttons with info from SERVER_LIST file... '${serverName} -- ${dbxCentralServerNameDescription}'
	...
	public List<CmHighlighterDescriptor> createHighlighterDescriptors()
	{
		...
		// ORANGE row: multi_sampled is not empty → seen in multiple samples
		list.add(new CmHighlighterDescriptor()
			.name("Multi-Sampled")
			.notEmpty("multi_sampled")
			.bgColor("#FFD480"));
