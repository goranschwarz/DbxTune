...
extends CountersModel
{
	...
	public static final String[] DIFF_COLUMNS     = new String[] {
			...
			"temp_blks_read",
			"temp_blks_written",
			"blk_read_time",         // renamed in 17 to 'shared_blk_read_time'
			"blk_write_time",        // renamed in 17 to 'shared_blk_write_time'
			"shared_blk_read_time",  // from: 17.0 ('blk_read_time'  renamed to 'shared_blk_read_time')
			"shared_blk_write_time", // from: 17.0 ('blk_write_time' renamed to 'shared_blk_write_time')
			"wal_records",           // from: 13.0
			"wal_fpi",               // from: 13.0   FPI = Full Page Image
			"wal_bytes",             // from: 13.0
			"temp_blk_read_time",    // 15.0
			"temp_blk_write_time",   // 15.0
...
//			"parallel_workers_launched",  // "Number of parallel workers actually launched"); 
			"generic_plan_calls",         // "Number of times the statement has been executed using a generic plan"); 
			"custom_plan_calls",          // "Number of times the statement has been executed using a custom plan"); 

			"pgss_dealloc"
	};
	
	/** Keep pg_stat_statements_info.dealloc column, which is refreshed in refreshGetData, if not yet supported by PH, the value will be -1 */
	private long _pgss_dealloc = -1;

	/** Keep pg_stat_statements_info.stats_reset column, which is refreshed in refreshGetData, if not yet supported by PH, the value will be null */
	private Timestamp _pgss_stats_reset = null;
...
//	RS> 19   blk_write_time      java.sql.Types.DOUBLE  float8           
	...
	// -------  14 -> 15 -- 10 new columns
	//                      new: temp_blk_read_time  -- Time spent reading temporary file blocks (ms).
	//                      new: temp_blk_write_time -- Time spent writing temporary file blocks (ms)
	//                      new: jit_functions, jit_generation_time, jit_inlining_count, jit_inlining_time, jit_optimization_count, jit_optimization_time, jit_emission_count, jit_emission_time
	// -------  15 -> 16 -- 0 new columns
	// -------  16 -> 17 -- 2 new columns
	//                      new: stats_since         -- Indicates the exact time when statistics began accumulating for this specific query row entry
	//                      new: minmax_stats_since  -- Indicates the time when the entry’s minimum and maximum timing counters (min_exec_time, max_exec_time, etc.) were last reset.
	// -------  17 -> 18 -- 3 new columns
	//                      new: parallel_workers_to_launch -- The total number of parallel workers that the planner requested or attempted to launch for the statement
	//                      new: parallel_workers_launched  -- The actual number of parallel workers that were successfully spun up and executed during the runtime of the statement.
	//                      new: wal_buffers_full           -- The number of times the execution of the statement was forced to wait because the write-ahead log (WAL) buffers were completely full. This helps diagnose heavy write-throughput bottlenecking
	// -------  18 -> 19 -- 2 new columns
	//                      new: generic_plan_calls         -- The number of times a generic plan was chosen and executed for the parameterized query.
	//                      new: custom_plan_calls          -- The number of times a custom plan was generated and executed specifically for the literal parameter values provided
...
//			queryid = "    ,md5(query) as queryid \n";
		...
		return  "select \n" +
				...
				total_time2 +
				"    ,s.* \n" +
				"    ,cast(0    as bigint)    AS \"pgss_dealloc\" \n" +
				"    ,cast(null as timestamp) AS \"pgss_stats_reset\" \n" +
...
//				"where s.calls > 1 \n" +
	...
	throws Exception
	{
		...
		// Reset
		_pgss_dealloc     = -1;
		_pgss_stats_reset = null;

		// Get some extra info: pg_stat_statements_info 
		if (rowCount > 0 && conn.getDbmsVersionNumber() >= Ver.ver(14))
		{
			String sql = "select * from pg_stat_statements_info";
			try (Statement stmnt = conn.createStatement(); ResultSet rs = stmnt.executeQuery(sql))
			{
				while(rs.next())
				{
					_pgss_dealloc     = rs.getLong     (1);
					_pgss_stats_reset = rs.getTimestamp(2);
				}
			}
			catch (SQLException ex)
			{
				_logger.warn("Problems executiong SQL=|" + sql + "| in refreshGetData(DbxConnection conn). Skipping and continuing. Caught: " + ex);
			}
		}
	...
	{
		int pgss_dealloc_pos     = newSample.findColumn("pgss_dealloc");
		int pgss_stats_reset_pos = newSample.findColumn("pgss_stats_reset");

		if (pgss_dealloc_pos == -1)
			return;

		for (int rowId = 0; rowId < newSample.getRowCount(); rowId++)
		{
			newSample.setValueAt(_pgss_dealloc,     rowId, pgss_dealloc_pos);
			newSample.setValueAt(_pgss_stats_reset, rowId, pgss_stats_reset_pos);
		}
	}
