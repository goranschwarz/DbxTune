 ...
 ******************************************************************************/
package com.dbxtune.cm.oracle.gui;
...
//	private JScrollPane      _graphPanelScroll;
	...
	private JTextField       _connectionsAbs_txt           = new JTextField();
	...
	private JLabel           _oldestOpenTranTime_lbl       = new JLabel();
	private JTextField       _oldestOpenTranTime_txt       = new JTextField();

	private JLabel           _blockingWaitThreshold_lbl    = new JLabel();
	private JTextField       _blockingWaitThreshold_txt    = new JTextField();

	private JLabel           _blockingWaitCount_lbl        = new JLabel();
	private JTextField       _blockingWaitCount_txt        = new JTextField();
	private JTextField       _blockingWaitCountDiff_txt    = new JTextField();

	private JLabel           _blockingWaitMaxSec_lbl        = new JLabel();
	private JTextField       _blockingWaitMaxSec_txt        = new JTextField();
	private JTextField       _blockingWaitMaxSecDiff_txt    = new JTextField();
...
//		_localServerName_lbl  .setText("Local server name");
		...
		tooltip = "This is the internal instance/server name in Oracle, taken from V$INSTANCE column INSTANCE_NAME";
		...
		tooltip = "This is the hostname where Oracle is running, taken from V$INSTANCE column HOST_NAME";
		...
		tooltip = "This is the Oracle version, taken from V$INSTANCE column VERSION";
		...
		tooltip = "Date and time that Oracle was started.";
		...
		tooltip = "Number of days that Oracle has been running for.";
		...
		_connectionsAbs_txt   .setToolTipText(tooltip);
		_connectionsAbs_txt   .setEditable(false);
		...
		tooltip = "Number of distinct User Names that is logged in to Oracle.";
		...
		tooltip = "Time (in seconds) that processes must have waited for blocking in order to be reported.";
		_blockingWaitThreshold_lbl.setText("Block wait threshold");
		_blockingWaitThreshold_lbl.setToolTipText(tooltip);
		_blockingWaitThreshold_txt.setToolTipText(tooltip);
		_blockingWaitThreshold_txt.setEditable(false);

		tooltip = "Number of processes that blocks others from working/continuing.";
		_blockingWaitCount_lbl        .setText("Block waits Count");
		_blockingWaitCount_lbl        .setToolTipText(tooltip);
		_blockingWaitCount_txt        .setToolTipText(tooltip);
		_blockingWaitCount_txt        .setEditable(false);
		_blockingWaitCountDiff_txt    .setEditable(false);
		_blockingWaitCountDiff_txt    .setToolTipText("The difference since previous sample.");
		_blockingWaitCountDiff_txt.setForeground(Color.BLUE);

		tooltip = "How many seconds has the user that blocks others been blocking for (only the 'max' blocker...).";
		_blockingWaitMaxSec_lbl        .setText("Block waits Sec");
		_blockingWaitMaxSec_lbl        .setToolTipText(tooltip);
		_blockingWaitMaxSec_txt        .setToolTipText(tooltip);
		_blockingWaitMaxSec_txt        .setEditable(false);
		_blockingWaitMaxSecDiff_txt    .setEditable(false);
		_blockingWaitMaxSecDiff_txt    .setToolTipText("The difference since previous sample.");
		_blockingWaitMaxSecDiff_txt.setForeground(Color.BLUE);

		tooltip = "Time (in seconds) that processes must have waited for blocking in order to be reported.";
		_oldestOpenTranThreshold_lbl.setText("Oldes Tran threshold");
		...
		tooltip = "Oldest Open Transaction, presented in seconds.";
		_oldestOpenTranSec_lbl.setText("Oldest Tran Seconds");
		...
		tooltip = "Oldest Open Transaction, presented in time when it started.";
		_oldestOpenTranTime_lbl.setText("Oldest Tran Time");
		_oldestOpenTranTime_lbl.setToolTipText(tooltip);
		_oldestOpenTranTime_txt.setToolTipText(tooltip);
		_oldestOpenTranTime_txt.setEditable(false);
...
//		_totalErrorsDiff_txt  .setToolTipText(tooltip);
		...
		panel.add(_daysRunning_txt,              "growx, wrap 20");
		
		panel.add(_connections_lbl,              "");
		panel.add(_connectionsAbs_txt,           "growx, split");
		...
		panel.add(_oldestOpenTranTime_lbl,       "");
		panel.add(_oldestOpenTranTime_txt,       "growx, wrap");
		
		panel.add(_blockingWaitThreshold_lbl,    "");
		panel.add(_blockingWaitThreshold_txt,    "growx, wrap");
		
		panel.add(_blockingWaitCount_lbl,        "");
		panel.add(_blockingWaitCount_txt,        "growx, split");
		panel.add(_blockingWaitCountDiff_txt,    "growx, wrap");
		
		panel.add(_blockingWaitMaxSec_lbl,       "");
		panel.add(_blockingWaitMaxSec_txt,       "growx, split");
		panel.add(_blockingWaitMaxSecDiff_txt,   "growx, wrap");
...
//
		...
		//----------------------------------------------
		int blockWaits = 0;
		int blockWaitsThreshold = 10;

		try { blockWaits = Integer.parseInt(_blockingWaitMaxSec_txt.getText()); }
		...
		if (blockWaits > blockWaitsThreshold)
		{
			...
			_blockingWaitMaxSecDiff_txt.setBackground(Color.RED);

			MainFrame.getInstance().setBlockingLocks(true, blockWaits);
...
//			_focusToBlockingTab.setVisible(true);
		...
		{
			_blockingWaitMaxSec_txt    .setBackground(_localServerName_txt.getBackground());
			_blockingWaitMaxSecDiff_txt.setBackground(_localServerName_txt.getBackground());
...
//
		...
		int oldestOpenTranInSecThreshold = 10;

		try { oldestOpenTranInSec = Integer.parseInt(_oldestOpenTranSec_txt.getText()); }
		catch (NumberFormatException ignore) {}
...
//			_focusToDatabasesTab_oldestOpenTran.setVisible(true);
		...
		{
			_oldestOpenTranSec_txt.setBackground(_localServerName_txt.getBackground());
...
//		}
	...
	public synchronized void clearSummaryData()
	{
		...
		_oldestOpenTranTime_txt     .setText("");
		_blockingWaitThreshold_txt  .setText("");
		_blockingWaitCount_txt      .setText("");
		_blockingWaitCountDiff_txt  .setText("");
		_blockingWaitMaxSec_txt     .setText("");
		_blockingWaitMaxSecDiff_txt .setText("");
