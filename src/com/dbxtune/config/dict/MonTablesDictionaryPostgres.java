...
// http://www.postgresql.org/docs/current/static/auto-explain.html   --- The auto_explain module provides a means for logging execution plans of slow statements automatically, without having to run EXPLAIN by hand. This is especially helpful for tracking down un-optimized queries in large applications.
			...
			mtd.addColumn("pg_stat_statements", "shared_blk_read_time",       "Total time the statement spent reading shared blocks, in milliseconds (if track_io_timing is enabled, otherwise zero)");
			mtd.addColumn("pg_stat_statements", "shared_blk_write_time",      "Total time the statement spent writing shared blocks, in milliseconds (if track_io_timing is enabled, otherwise zero)");
			mtd.addColumn("pg_stat_statements", "local_blk_read_time",        "Total time the statement spent reading local blocks, in milliseconds (if track_io_timing is enabled, otherwise zero)");
			mtd.addColumn("pg_stat_statements", "local_blk_write_time",       "Total time the statement spent writing local blocks, in milliseconds (if track_io_timing is enabled, otherwise zero)");
			...
			mtd.addColumn("pg_stat_statements", "wal_buffers_full",           "Number of times the WAL buffers became full");
			...
			mtd.addColumn("pg_stat_statements", "jit_deform_count",           "Total number of tuple deform functions JIT-compiled by the statement"); 
			mtd.addColumn("pg_stat_statements", "jit_deform_time",            "Total time spent by the statement on JIT-compiling tuple deform functions, in milliseconds"); 
			mtd.addColumn("pg_stat_statements", "parallel_workers_to_launch", "Number of parallel workers planned to be launched"); 
			mtd.addColumn("pg_stat_statements", "parallel_workers_launched",  "Number of parallel workers actually launched"); 
			mtd.addColumn("pg_stat_statements", "generic_plan_calls",         "Number of times the statement has been executed using a generic plan"); 
			mtd.addColumn("pg_stat_statements", "custom_plan_calls",          "Number of times the statement has been executed using a custom plan"); 
			mtd.addColumn("pg_stat_statements", "stats_since",                "Time at which statistics gathering started for this statement"); 
			mtd.addColumn("pg_stat_statements", "minmax_stats_since",         "Time at which min/max statistics gathering started for this statement (fields min_plan_time, max_plan_time, min_exec_time and max_exec_time)"); 
			
			mtd.addColumn("pg_stat_statements", "pgss_dealloc",               "pg_stat_statements_info: Total number of times pg_stat_statements entries about the least-executed statements were deallocated because more distinct statements than pg_stat_statements.max were observed");
			mtd.addColumn("pg_stat_statements", "pgss_stats_reset",           "pg_stat_statements_info: Time at which all statistics in the pg_stat_statements view were last reset.");
