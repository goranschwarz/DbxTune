...
/** Extend the JXTable */
public class AlarmWriterFiltersTable extends JXTable
{
	private static final long serialVersionUID = 0L;
	private static Logger _logger = Logger.getLogger(AlarmWriterFiltersTable.class);
...
//	AlarmWriterFiltersPanel       _alarmWriterFiltersPanel;
	...
	public AlarmWriterFiltersTable(AlarmWriterFiltersTableModel alarmWriterFiltersTableModel)
//	public AlarmWriterFiltersTable(AlarmWritersTableModel alarmWritersTableModel, AlarmWriterFiltersPanel alarmWriterFiltersPanel)
	{
		super();
		
		_alarmWriterFiltersTableModel   = alarmWriterFiltersTableModel;

		setModel(_alarmWriterFiltersTableModel);
...
//		setHighlighters(_highliters);
		...
		addHighlighter( new ColorHighlighter(new HighlightPredicate()
		{
			...
			{
				if (AlarmWriterFiltersTableModel.TAB_POS_VALUE == convertColumnIndexToModel(adapter.column))
				{
					CmSettingsHelper awse = _alarmWriterFiltersTableModel.getSettingForRow(convertRowIndexToModel(adapter.row));
		...
		addHighlighter( new ColorHighlighter(new HighlightPredicate()
		{
			...
			public boolean isHighlighted(Component renderer, ComponentAdapter adapter)
			{
				if (AlarmWriterFiltersTableModel.TAB_POS_IS_DEFAULT == convertColumnIndexToModel(adapter.column))
				{
					CmSettingsHelper awse = _alarmWriterFiltersTableModel.getSettingForRow(convertRowIndexToModel(adapter.row));
		...
		addHighlighter( new ColorHighlighter(new HighlightPredicate()
		{
			...
			public boolean isHighlighted(Component renderer, ComponentAdapter adapter)
			{
				if (AlarmWriterFiltersTableModel.TAB_POS_PROPERTY_KEY == convertColumnIndexToModel(adapter.column))
				{
					CmSettingsHelper awse = _alarmWriterFiltersTableModel.getSettingForRow(convertRowIndexToModel(adapter.row));
	...
	protected JTableHeader createDefaultTableHeader()
	{
		JTableHeader tabHeader = new JXTableHeader(getColumnModel())
		{
			...
			public String getToolTipText(MouseEvent e)
			{
				...
				tip = _alarmWriterFiltersTableModel.getToolTipText(mcol);
...
//	public void refreshTable(List<AlarmWriterFiltersEntry> filters)
//	{
//		_alarmWriterFiltersTableModel.refreshTable(filters);
//		packAll(); // set size so that all content in all cells are visible
//	}
	public void refreshTable(List<CmSettingsHelper> filters, String writerClassName)
	{
		...
		Runnable doLater = new Runnable()
		{
			...
			public void run()
			{
				...
				int colPos = convertColumnIndexToView(AlarmWriterFiltersTableModel.TAB_POS_VALUE);
				if (colPos >= 0)
				{
				...
				// Set "template" columns to max a max width...
				colPos = convertColumnIndexToView(AlarmWriterFiltersTableModel.TAB_POS_DEFAULT);
				if (colPos >= 0)
				{
					TableColumnExt tce = getColumnExt(colPos);
					if (tce.getWidth() > maxWidth)
	...
	private Configuration getCurrentConfig()
	{
		...
		for (CmSettingsHelper sh : _alarmWriterFiltersTableModel.getSettings())
		{
	...
	*/
	...
	public JPopupMenu createDataTablePopupMenu()
	{
		...
		edit.addActionListener(new ActionListener()
		{
			...
			{
				JTable table = AlarmWriterFiltersTable.this;
...
//				String val = getModel().getValueAt(mrow, mcol) + "";
	...
	{
		return _alarmWriterFiltersTableModel.isDirty();
	}
