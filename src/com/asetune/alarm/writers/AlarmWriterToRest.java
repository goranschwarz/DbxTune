...
extends AlarmWriterAbstract
{
	...
	private void sendMessage(String action, AlarmEvent alarmEvent)
	{
		...
		try
		{
			...
			{
				_logger.info("Responce code "+responceCode+" ("+htmlResponceCodeToText(responceCode)+"). From URL '"+_url+"'. Sent JSON content: "+jsonMessage);
			}
	...
	/**
	 * Get a descriptive text for a HTTP responce code<br>
	 * from https://httpstatuses.com/
	 * 
	 * @param code
	 */
	public static String htmlResponceCodeToText(int code)
	{
		switch (code)
		{
		// --------------------------------------------------------
