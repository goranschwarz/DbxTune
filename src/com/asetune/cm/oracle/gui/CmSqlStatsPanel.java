...
//	private static final Logger  _logger	           = Logger.getLogger(CmPlanCacheDetailsPanel.class);
	...
	public static final String  TOOLTIP_sample_lastXminutes = 
		...
		+ "<b>Note:</b> Number of minutes can be changed using the property: <code>"+CmSqlStats.PROPKEY_sample_lastXminutesTime+"</code><br>"
		+ "</html>";
...
//		"<br>" +
	...
	public CmSqlStatsPanel(CountersModel cm)
	{
		super(cm);
...
//		_sampleLastXminutes_chk     = new JCheckBox("Show only SQL executed last 10 minutes",         conf == null ? CmSqlStats.DEFAULT_sample_lastXminutes    : conf.getBooleanProperty(CmSqlStats.PROPKEY_sample_lastXminutes,     CmSqlStats.DEFAULT_sample_lastXminutes));
		_sampleLastXminutes_chk     = new JCheckBox("Show only SQL executed last",                    conf == null ? CmSqlStats.DEFAULT_sample_lastXminutes    : conf.getBooleanProperty(CmSqlStats.PROPKEY_sample_lastXminutes,     CmSqlStats.DEFAULT_sample_lastXminutes));
		_sampleLastXminutes_txt     = new JTextField(""+                                             (conf == null ? CmSqlStats.DEFAULT_sample_lastXminutesTime: conf.getIntProperty    (CmSqlStats.PROPKEY_sample_lastXminutesTime, CmSqlStats.DEFAULT_sample_lastXminutesTime)), 3);
		_sampleLastXminutes_lbl     = new JLabel("minutes");
		_sampleAfterPrevSample_chk  = new JCheckBox("Show only SQL executed since last sample time",  conf == null ? CmSqlStats.DEFAULT_sample_afterPrevSample : conf.getBooleanProperty(CmSqlStats.PROPKEY_sample_afterPrevSample,  CmSqlStats.DEFAULT_sample_afterPrevSample));
		...
		String sampleExtraWhereClause = (conf == null ? CmSqlStats.DEFAULT_sample_extraWhereClause : conf.getProperty(CmSqlStats.PROPKEY_sample_extraWhereClause, CmSqlStats.DEFAULT_sample_extraWhereClause));
		...
		_sampleLastXminutes_chk.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if (conf == null) return;
				conf.setProperty(CmSqlStats.PROPKEY_sample_lastXminutes, ((JCheckBox)e.getSource()).isSelected());
				conf.save();
		...
		_sampleLastXminutes_txt.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if (sampleTimeInt < 0)
				{
					SwingUtils.showErrorMessage(CmSqlStatsPanel.this, "Not a Number", "This must be specified as an integer value, and it was '"+sampleTimeStr+"'.", null);
					_sampleLastXminutes_txt.setText(CmSqlStats.DEFAULT_sample_lastXminutesTime + "");
					return;
				}
				conf.setProperty(CmSqlStats.PROPKEY_sample_lastXminutesTime, _sampleLastXminutes_txt.getText());
				conf.save();
		...
		_sampleAfterPrevSample_chk.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				Configuration conf = Configuration.getInstance(Configuration.USER_TEMP);
				if (conf == null) return;
				conf.setProperty(CmSqlStats.PROPKEY_sample_afterPrevSample, ((JCheckBox)e.getSource()).isSelected());
				conf.save();
		...
		_sampleExtraWhereClause_but.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				conf.setProperty(CmSqlStats.PROPKEY_sample_extraWhereClause, extraWhere);
				conf.save();
		...
		_sampleExtraWhereToDef_but.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				_sampleExtraWhereClause_txt.setText(CmSqlStats.DEFAULT_sample_extraWhereClause);
