...
//	private static final Logger  _logger	           = Logger.getLogger(CmMemoryClerksPanel.class);
	...
	private static final String  PROPKEY_generateOthersPct     = PROP_PREFIX + ".graph.generate.lt.pct.to.others";
	private static final double  DEFAULT_generateOthersPct     = 0.05; // 0.05% or less goes to "Others"

	private static final String  PROPKEY_generateSizeMb        = PROP_PREFIX + ".graph.generate.SizeMb";
	private static final boolean DEFAULT_generateSizeMb        = true;
...
//	private static final boolean DEFAULT_generateCpuUsagePct         = true;
	...
	static
	{
		...
		Configuration.registerDefaultValue(PROPKEY_generateSizeMb,              DEFAULT_generateSizeMb);
...
//		Configuration.registerDefaultValue(PROPKEY_generateCpuUsagePct,         DEFAULT_generateCpuUsagePct);
	...
	public CmMemoryClerksPanel(CountersModel cm)
	{
		super(cm);
...
//			setIcon( SwingUtils.readImageIcon(Version.class, cm.getIconFile()) );
	...
	private CategoryDataset createDataset(GTable dataTable)
	{
		...
		boolean generateSizeMb              = conf.getBooleanProperty(PROPKEY_generateSizeMb,              DEFAULT_generateSizeMb);
...
//		boolean generateCpuUsagePct         = conf.getBooleanProperty(PROPKEY_generateCpuUsagePct,         DEFAULT_generateCpuUsagePct);
		...
		if (dataTable != null)
		{
			int type_pos                = dataTable.findViewColumn("type");
			int SizeMb_pos              = dataTable.findViewColumn("SizeMb");
...
//			double CpuUsagePctSUM = 0; // Sum of ALL CpuUsagePct rows... then we can calculate "idle" 
			if (cm != null)
			{
				...
				{
					String OperationName       = (String)dataTable.getValueAt(r, type_pos);
					Number SizeMb              = (Number)dataTable.getValueAt(r, SizeMb_pos);
...
//			generateDummy = true;
			if (generateDummy)
			{
				dataset.addValue(1,   "Unknown",     "SizeMb");
				dataset.addValue(2,   "Execution",   "SizeMb");
				dataset.addValue(3,   "Sorting",     "SizeMb");
				dataset.addValue(4,   "Compilation", "SizeMb");
	...
	private JFreeChart createChart(CategoryDataset dataset)
	{
		...
		final String          chartTitle              = "Memory Clerks Graph";
...
//			plot.setLimit(0.005);  // 0.5%  or less goes to "Others"
//			plot.setLimit(0.001);  // 0.1%  or less goes to "Others"
//			plot.setLimit(0.0005); // 0.05% or less goes to "Others"
			
			double otherLimit = Configuration.getCombinedConfiguration().getDoubleProperty(PROPKEY_generateOthersPct, DEFAULT_generateOthersPct) / 100.0; // DEFAULT: 0.05
			plot.setLimit(otherLimit); // DEFAULT = 0.0005 ===>> 0.05% or less goes to "Others"
...
//		    numAxis.setUpperBound(1); // 100%
	...
	protected JPanel createLocalOptionsPanel()
	{
		...
		final JCheckBox enableSizeMb_chk        = new JCheckBox("SizeMb");
...
//		final JCheckBox enableCpuUsagePct_chk         = new JCheckBox("CpuUsagePct");
		...
		enableSizeMb_chk        .setToolTipText("Show SizeMb per type");
...
//		enableCpuUsagePct_chk         .setToolTipText("Show CPU Usage per 'operations category'");
		...
		enableSizeMb_chk       .setSelected(conf.getBooleanProperty(PROPKEY_generateSizeMb,        DEFAULT_generateSizeMb));
...
//		enableCpuUsagePct_chk        .setSelected(conf.getBooleanProperty(PROPKEY_generateCpuUsagePct,         DEFAULT_generateCpuUsagePct));
		...
		enableSizeMb_chk.addActionListener(new ActionListener()
		{
			@Override
			public void actionPerformed(ActionEvent e)
			{
				helperActionSave(PROPKEY_generateSizeMb, ((JCheckBox)e.getSource()).isSelected());
			}
		});
...
//		});
		...
		panel.add(enableSizeMb_chk,        "split");
