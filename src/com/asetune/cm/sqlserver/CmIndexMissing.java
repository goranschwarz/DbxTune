...
//			    + "    , TableName   = object_name(mid.object_id, mid.database_id) \n"
				+ "    , SchemaName  = (select sys.schemas.name from sys.objects inner join sys.schemas ON sys.schemas.schema_id = sys.objects.schema_id where sys.objects.object_id = mid.object_id) \n"
				+ "    , TableName   = (select sys.objects.name from sys.objects where sys.objects.object_id = mid.object_id) \n"
...
//			    + "    , CreateIndexStatement = 'CREATE NONCLUSTERED INDEX ' + object_name(mid.object_id, mid.database_id) + '_xxx_ix' + convert(varchar(10), mid.index_handle) \n"
			    + "    , CreateIndexStatement = 'CREATE NONCLUSTERED INDEX ' + (select sys.objects.name from sys.objects where sys.objects.object_id = mid.object_id) COLLATE DATABASE_DEFAULT + '_xxx_ix' + convert(varchar(10), mid.index_handle) \n"
			    + "         + ' ON ' + mid.statement + ' ' \n"
