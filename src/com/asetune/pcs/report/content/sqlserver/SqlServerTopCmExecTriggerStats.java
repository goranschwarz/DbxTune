...
public class SqlServerTopCmExecTriggerStats
extends SqlServerAbstract
{
	private static Logger _logger = Logger.getLogger(SqlServerTopCmExecTriggerStats.class);

	private ResultSetTableModel _shortRstm;
	private ExecutionPlanCollection _planCollection;
	private List<String>        _miniChartJsList = new ArrayList<>();

	public SqlServerTopCmExecTriggerStats(DailySummaryReportAbstract reportingInstance)
	{
...
//		sb.append("Row Count: " + _shortRstm.getRowCount() + "<br>\n");
	...
	{
		return "Top Trigger Calls (order by: total_worker_time, origin: CmExecTriggerStats/dm_exec_trigger_stats)";
	}
	...
	{
		return new String[] { "CmExecTriggerStats_diff" };
	}
	...
	public void create(DbxConnection conn, String srvName, Configuration pcsSavedConf, Configuration localConf)
	{
		...
		String dummySql = "select * from [CmExecTriggerStats_diff] where 1 = 2";
		ResultSetTableModel dummyRstm = executeQuery(conn, dummySql, false, "metadata");
		if (dummyRstm == null)
		{
			String msg = "Table 'CmExecTriggerStats_diff' did not exist. So Performance Counters for this hasn't been sampled during this period.";

			//addMessage(msg);
			setProblemException(new Exception(msg));

			_shortRstm = ResultSetTableModel.createEmpty("CmExecTriggerStats");
			return;
		}
...
//		RS> 30   total_spills                java.sql.Types.BIGINT    bigint            -none-      
//		RS> 31   last_spills                 java.sql.Types.BIGINT    bigint            -none-      
//		RS> 32   min_spills                  java.sql.Types.BIGINT    bigint            -none-      
//		RS> 33   max_spills                  java.sql.Types.BIGINT    bigint            -none-      
		
//		RS> 34   total_num_physical_reads    java.sql.Types.BIGINT    bigint            -none-      
//		RS> 35   last_num_physical_reads     java.sql.Types.BIGINT    bigint            -none-      
//		RS> 36   min_num_physical_reads      java.sql.Types.BIGINT    bigint            -none-      
//		RS> 37   max_num_physical_reads      java.sql.Types.BIGINT    bigint            -none-      
		
//		RS> 38   total_page_server_reads     java.sql.Types.BIGINT    bigint            -none-      
//		RS> 39   last_page_server_reads      java.sql.Types.BIGINT    bigint            -none-      
//		RS> 40   min_page_server_reads       java.sql.Types.BIGINT    bigint            -none-      
//		RS> 41   max_page_server_reads       java.sql.Types.BIGINT    bigint            -none-      
		
//		RS> 42   total_num_page_server_reads java.sql.Types.BIGINT    bigint            -none-      
//		RS> 43   last_num_page_server_reads  java.sql.Types.BIGINT    bigint            -none-      
//		RS> 44   min_num_page_server_reads   java.sql.Types.BIGINT    bigint            -none-      
//		RS> 45   max_num_page_server_reads   java.sql.Types.BIGINT    bigint            -none-      
...
//		String col_total_columnstore_segment_skips__sum = !dummyRstm.hasColumnNoCase("total_columnstore_segment_skips") ? "" : "    ,sum([total_columnstore_segment_skips]) as [total_columnstore_segment_skips__sum]   \n"; 
		...
		String col_total_num_physical_reads__sum        = !dummyRstm.hasColumnNoCase("total_num_physical_reads"       ) ? "" : "    ,sum([total_num_physical_reads])        as [total_num_physical_reads__sum]          \n"; 
		...
		String col_total_num_page_server_reads__sum     = !dummyRstm.hasColumnNoCase("total_num_page_server_reads"    ) ? "" : "    ,sum([total_num_page_server_reads])     as [total_total_num_page_server_reads__sum] \n"; 
...
//		String col_AvgNumPhysicalReads                  = !dummyRstm.hasColumnNoCase("total_num_physical_reads"       ) ? "" : "    ,cast(-1 as numeric(16,1)) as [AvgNumPhysicalReads]        \n";
...
//		String col_AvgNumPageServerReads                = !dummyRstm.hasColumnNoCase("total_num_page_server_reads"    ) ? "" : "    ,cast(-1 as numeric(16,1)) as [AvgNumPageServerReads]      \n"; 
...
//		String col_AvgColumnstoreSegmentSkips           = !dummyRstm.hasColumnNoCase("total_columnstore_segment_skips") ? "" : "    ,cast( sum([total_columnstore_segment_skips])                * 1.0 / nullif(sum([execution_count]), 0) as numeric(19,1)) as [AvgColumnstoreSegmentSkips] \n"; 
		...
		String col_AvgNumPhysicalReads                  = !dummyRstm.hasColumnNoCase("total_num_physical_reads"       ) ? "" : "    ,cast( sum([total_num_physical_reads])                       * 1.0 / nullif(sum([execution_count]), 0) as numeric(19,1)) as [AvgNumPhysicalReads]        \n";
		...
		String col_AvgNumPageServerReads                = !dummyRstm.hasColumnNoCase("total_num_page_server_reads"    ) ? "" : "    ,cast( sum([total_num_page_server_reads])                    * 1.0 / nullif(sum([execution_count]), 0) as numeric(19,1)) as [AvgNumPageServerReads]      \n"; 
...
//		if (dummyRstm.hasColumnNoCase("total_worker_time"))     { orderByCol = "[total_worker_time__sum]";     }
		...
		try {
			hasDictCompCols = DictCompression.hasCompressedColumnNames(conn, null, "CmExecTriggerStats_diff");
		} catch (SQLException ex) {
			_logger.error("Problems checking for Dictionary Compressed Columns in table 'CmExecTriggerStats_diff'.", ex);
		}
		...
		String sql = getCmDiffColumnsAsSqlComment("CmExecTriggerStats")
			    + "select top " + topRows + " \n"
			    + "     [DbName] \n"
			    + "    ,[SchemaName] \n"
			    + "    ,[TableName] \n"
...
//			    + col_total_columnstore_segment_skips__sum + col_AvgColumnstoreSegmentSkips 
			    ...
			    + col_total_num_physical_reads__sum        + col_AvgNumPhysicalReads                  
			    + col_total_page_server_reads__sum         + col_AvgPageServerReads         
			    + col_total_num_page_server_reads__sum     + col_AvgNumPageServerReads         
			    + "    \n"
...
//				+ "  and [TotalIOs]   > " + _aboveTotalIos    + " \n"
		...
		_shortRstm = executeQuery(conn, sql, false, "CmExecTriggerStats");
		if (_shortRstm == null)
		{
			_shortRstm = ResultSetTableModel.createEmpty("CmExecTriggerStats");
			return;
...
//			calculateAvg(_shortRstm);
			...
			// Mini Chart on "..."
			//-----------------------------------------------
			String whereKeyColumn = "DbName, SchemaName, TableName"; 
			...
			_miniChartJsList.add(SparklineHelper.createSparkline(conn, this, _shortRstm, 
					...
					.setDbmsTableName            ("CmExecTriggerStats_diff")
...
//			calculateAvg(rstm, r, "execution_count__sum", "total_num_physical_reads"            , "AvgNumPhysicalReads");        
//			calculateAvg(rstm, r, "execution_count__sum", "total_page_server_reads__sum"        , "AvgPageServerReads");        
//			calculateAvg(rstm, r, "execution_count__sum", "total_num_page_server_reads__sum"    , "AvgNumPageServerReads");        
//		}
//	}
//
...
//	}
	...
	private void setSectionDescription(ResultSetTableModel rstm)
	{
		...
		rstm.setDescription(
				...
				"<br>" +
				"SqlServer Source table is 'dm_exec_trigger_stats'. <br>" +
				"PCS Source table is 'CmExecTriggerStats_diff'. (PCS = Persistent Counter Store) <br>" +
				"");
	}
}
