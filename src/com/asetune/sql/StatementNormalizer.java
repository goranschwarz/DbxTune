...
public class StatementNormalizer
{
	 ...
	 */
	public static boolean isNotYetSupportedByParser(String sql)
	{
		if (sql == null) return true;

		// DUMP TRAN|DATABASE ...
		if (StringUtils.startsWithIgnoreCase(sql, "dump ")) return true;

		// set nocount on Any-SQL-stmnt ... 
		if (StringUtils.startsWithIgnoreCase(sql, "set nocount ")) return true;
		if (StringUtils.startsWithIgnoreCase(sql, "\nset nocount ")) return true;

		// BULK Inserts: insert bulk dbname..tabname with arrayinsert, nodescribe
		if (StringUtils.startsWithIgnoreCase(sql, "insert bulk ")) return true;

		// COMMIT TRANSACTION ...
		// This seems strange...
		if (StringUtils.startsWithIgnoreCase(sql, "commit transaction ")) return true;

		// writetext
		if (StringUtils.startsWithIgnoreCase(sql, "writetext ")) return true;
		
		// OPEN_CURSOR
		if (StringUtils.startsWithIgnoreCase(sql, "OPEN_CURSOR ")) return true;
