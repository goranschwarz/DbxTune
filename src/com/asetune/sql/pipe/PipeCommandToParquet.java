...
//		String  _rowTermReadable;   // set in parse()
	...
	public PipeCommandToParquet(String input, String sqlString, ConnectionProvider connProvider)
	...
	throws PipeCommandException
	{
		...
		if (notYetImplemented)
			throw new PipeCommandException("PipeCommand, cmd='" + input + "' is -NOT-YET-IMPLEMENTED-");

		
		if (input.startsWith("toparquet ") || input.equals("toparquet"))
		{
...
//			_params._rowTermReadable   = StringUtil.escapeControlChars(_params._rowTerm);
			...
			System.out.println("TOPARQUET Param: _append        = '"+ _params._append            + "'.");
			System.out.println("TOPARQUET Param: _overwrite     = '"+ _params._overwrite         + "'.");
//			System.out.println("TOPARQUET Param: _fieldTerm     = '"+ _params._fieldTermReadable + "'.");
//			System.out.println("TOPARQUET Param: _rowTerm       = '"+ _params._rowTermReadable   + "'.");
//			System.out.println("TOPARQUET Param: _nullValue     = '"+ _params._nullValue         + "'.");
//			System.out.println("TOPARQUET Param: _charset       = '"+ _params._charset           + "'.");
//			System.out.println("TOPARQUET Param: _trimValues    = '"+ _params._trimValues        + "'.");
			System.out.println("TOPARQUET Param: _queryInfo     = '"+ _params._queryInfo         + "'.");
			System.out.println("TOPARQUET Param: _rsInfo        = '"+ _params._rsInfo            + "'.");
			System.out.println("TOPARQUET Param: _noGuiQuestion = '"+ _params._noGuiQuestion     + "'.");
		}
		else
		{
			throw new PipeCommandException("PipeCommand, cmd='"+input+"' is unknown. Available commands is: toparquet");
		}
	}
...
//		sb.append("  \n");
		...
		sb.append("usage: toparquet [-a] [-o] [-h] [-f <str>] [-r <str>] [-c <charset>] [-q] [-i] [-n] [-R] filename\n");
		sb.append("   \n");
		sb.append("description: \n");
		sb.append("  Write the ResultSet directly to a Parquet File instead of presenting all records in the GUI.\n");
		sb.append("   \n");
		sb.append("options: \n");
		sb.append("  -a,--append                  Append to file.\n");
		sb.append("  -o,--overwrite               If file already exists overwrite it.\n");
...
//			_logger.info("Opened the file '"+_outfile+"' to write data. Options: append="+_cmdParams._append+", overwrite="+_cmdParams._overwrite+", header="+_cmdParams._header+", fieldTerm='"+_cmdParams._fieldTermReadable+"', rowTerm='"+_cmdParams._rowTermReadable+"', charset='"+_cmdParams._charset+"', queryInfo='"+_cmdParams._queryInfo+"', rsInfo='"+_cmdParams._rsInfo+"', noGuiQuestion='"+_cmdParams._noGuiQuestion+"'.");
			...
			_logger.info("Opened the file '"+_outfile+"' to write data. Options: append="+_cmdParams._append+", overwrite="+_cmdParams._overwrite+", header="+_cmdParams._header+", queryInfo='"+_cmdParams._queryInfo+"', rsInfo='"+_cmdParams._rsInfo+"', noGuiQuestion='"+_cmdParams._noGuiQuestion+"'.");
		}
		...
		public int doTransfer(ResultSet sourceRs, PipeCommandToParquet pipeCmd)
		throws Exception
		{
			int sourceNumCols = -1;
...
//			"Using options: append="+_cmdParams._append+", overwrite="+_cmdParams._overwrite+", header="+_cmdParams._header+", fieldTerm='"+_cmdParams._fieldTermReadable+"', rowTerm='"+_cmdParams._rowTermReadable+"', charset='"+_cmdParams._charset+"', trim="+_cmdParams._trimValues+", queryInfo='"+_cmdParams._queryInfo+"', rsInfo='"+_cmdParams._rsInfo+"', noGuiQuestion='"+_cmdParams._noGuiQuestion+"'.";
			pipeCmd._message = "Successfully wrote "+totalCount+" row(s) to file '"+_outfile+"'.\n" + 
			"Using options: append="+_cmdParams._append+", overwrite="+_cmdParams._overwrite+", header="+_cmdParams._header+", queryInfo='"+_cmdParams._queryInfo+"', rsInfo='"+_cmdParams._rsInfo+"', noGuiQuestion='"+_cmdParams._noGuiQuestion+"'.";
