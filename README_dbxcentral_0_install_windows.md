# Installation instructions for DbxCentral on Windows
...
## Add it as a local administrator
...
* Something that allows username `dbxtune` user to Start/Stop Services
* Possibly also to create services


**IMPORTANT**: The user needs to have role `Create symbolic links`   
...
The above is harder to do on the command line but if you got `ntrights`, which is part of **Windows Resource Kit** you can do the following    
```
ntrights +r SeCreateSymbolicLinkPrivilege -u dbxtune
```

Add DbxTune user to group: "Performance Log Users"    
_This because DbxCentral issues `typeperf ...` to sample local metrics on the machine that hosts DbxCentral_    
```
## Allow user to get perf counters by issuing 'typeperf ...'
net localgroup "Performance Log Users" dbxtune /add
```

Then Execute `gpupdate /force` to update the policies    

To check if the dbxtune user can create symbolic links:
  - open:  `runas /user:dbxtune cmd`
  - check: `whoami /priv | findstr SeCreateSymbolicLinkPrivilege`
  - test to create a symlink:
    - `cd c:\Users\dbxtune`
    - `echo xxx > dummy.txt`
    - `mklink link_to_dummy.txt dummy.txt` 
    - `dir`
    - `more link_to_dummy.txt`
    - `del link_to_dummy.txt dummy.txt`
    - `dir`
...
```
or in mklink in powershell: `New-Item -Path 0 -ItemType SymbolicLink -Value dbxtune_2025-03-02`
...
The below command creates directory `c:\Users\dbxtune\.dbxtune` and copies some files to it

```
cd c:\Users\dbxtune\dbxtune\0\bin
dbxcentral.bat --createAppDir
```
...
- To *link* the `data` and `report` directory to another location, do the below:
	```
	cd c:\Users\dbxtune\.dbxtune\dbxc

	rmdir data
	mklink /D data x:\path\with\much\space\dbxtune\data
	dir data

	rmdir reports
  mklink /D reports x:\path\with\much\space\dbxtune\reports
	dir reports
	```
...
-- FIXME --    
Possibly: `netsh interface portproxy add v4tov4 listenport=4422 listenaddress=192.168.1.111 connectport=80 connectaddress=192.168.0.33`
`
...
- This is done with a shell script `start_dbxcentral.sh` found at `${HOME}/.dbxtune/dbxc/bin`
	...
	Example:
	```
	cd c:\Users\dbxtune\.dbxtune\dbxc\bin
	start_dbxcentral.bat
	```
...
if it **doesn't** work:
 - Check: more `c:\Users\dbxtune\.dbxtune\dbxc\log\DBX_CENTRAL.console`
 - Check: firewall, so that it allows incoming on port `8080`
